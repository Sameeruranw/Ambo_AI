import "./style.css";
import { jsx as l, jsxs as G, Fragment as sA } from "react/jsx-runtime";
import * as Q from "react";
import { forwardRef as oA, createContext as bA, useContext as hA, Fragment as ZA, useRef as tA, useState as H, useCallback as O, useEffect as K, createElement as tt, isValidElement as ct, useMemo as fA } from "react";
import kt from "react-dom/server";
const J = {
  AUDIO_BUTTON: "audio-button",
  NOTIFICATION_BUTTON: "notification-button",
  CLOSE_CHAT_BUTTON: "close-chat-button",
  SEND_MESSAGE_BUTTON: "send-button",
  VOICE_MESSAGE_BUTTON: "voice-button",
  FILE_ATTACHMENT_BUTTON: "file-attachment-button",
  EMOJI_PICKER_BUTTON: "emoji-button"
}, Lt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", width: 32, height: 32, viewBox: "0 0 42 42", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { style: {
  fill: "none",
  strokeWidth: 2,
  strokeLinecap: "butt",
  strokeLinejoin: "miter",
  stroke: "#b00000",
  strokeOpacity: 1,
  strokeMiterlimit: 4
}, d: "M17 9a8 8 0 0 1-8 8 8 8 0 0 1-8-8 8 8 0 0 1 8-8 8 8 0 0 1 8 8Zm0 0", transform: "rotate(.086) scale(1.77778)" }), /* @__PURE__ */ Q.createElement("path", { style: {
  fill: "none",
  strokeWidth: 2,
  strokeLinecap: "butt",
  strokeLinejoin: "miter",
  stroke: "#b00000",
  strokeOpacity: 1,
  strokeMiterlimit: 10
}, d: "M14.66 14.66 3.34 3.34", transform: "rotate(.086) scale(1.77778)" }));
oA(Lt);
const xt = "data:image/svg+xml;base64,", Rt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", xmlSpace: "preserve", viewBox: "0 0 1000 1000", ref: t, ...A }, /* @__PURE__ */ Q.createElement("rect", { width: "100%", height: "100%", fill: "#fff" }), /* @__PURE__ */ Q.createElement("g", null, /* @__PURE__ */ Q.createElement("rect", { width: 45, height: 30, x: -22.5, y: -15, rx: 1.5, ry: 1.5, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#37547a",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(90 -99 362) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("path", { d: "M0 75q14 0 14 17 0 16-14 16Z", style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#98b0ca",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(180 106 386) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 25, height: 4, x: -12.5, y: -2, rx: 0.2, ry: 0.2, style: {
  stroke: "#46648c",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#46648c",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(90 -46 282) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("path", { d: "M19 65h4l5 4v1H14v-1Z", style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#46648c",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(165 150) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 25, height: 2, x: -12.5, y: -1, rx: 0.1, ry: 0.1, style: {
  stroke: "#98b0ca",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#98b0ca",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(90 -3 240) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("circle", { r: 7.5, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#a478fc",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(236 175) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 45, height: 30, x: -22.5, y: -15, rx: 1.5, ry: 1.5, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#37547a",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(90 139 600) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("path", { d: "M184 75q14 0 14 17 0 16-14 16Z", style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#98b0ca",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(165 150) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 25, height: 4, x: -12.5, y: -2, rx: 0.2, ry: 0.2, style: {
  stroke: "#46648c",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#46648c",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(90 219 548) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("path", { d: "M175 65h4l5 4v1h-14v-1Z", style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#46648c",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(165 150) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 25, height: 2, x: -12.5, y: -1, rx: 0.1, ry: 0.1, style: {
  stroke: "#98b0ca",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#98b0ca",
  fillRule: "nonzero",
  opacity: 1
}, transform: "rotate(90 262 505) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("circle", { r: 7.5, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#a478fc",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(767 175) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 116.7, height: 70, x: -58.4, y: -35, rx: 31.5, ry: 31.5, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#71c9fc",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(502 877) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 60, height: 20, x: -30, y: -10, rx: 3, ry: 3, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#37547a",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(502 728) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("circle", { r: 75, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#71c9fc",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(502 473) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 129.4, height: 82.3, x: -64.7, y: -41.1, rx: 37, ry: 37, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#fff",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(502 464) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 108, height: 68.6, x: -54, y: -34.3, rx: 30.9, ry: 30.9, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#162334",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(502 464) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("circle", { r: 19.4, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#a478fc",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(415 464) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("circle", { r: 19.4, style: {
  stroke: "none",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#a478fc",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(591 464) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("rect", { width: 96, height: 53.2, x: -48, y: -26.6, rx: 23.9, ry: 23.9, style: {
  stroke: "#162334",
  strokeWidth: 1,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 4,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "#162334",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(503 875) scale(3.4003)" }), /* @__PURE__ */ Q.createElement("path", { strokeLinecap: "round", d: "m68 215 7-15M81 222l-7-22M81 222l7-14M101 208H87M114 219l-7-24M113 219l13-22M131 214l-5-17M148 214h-18M69 214H51M100 209l7-15", style: {
  stroke: "#a478fc",
  strokeWidth: 3,
  strokeDasharray: "none",
  strokeLinecap: "butt",
  strokeDashoffset: 0,
  strokeLinejoin: "miter",
  strokeMiterlimit: 10,
  isCustomFont: "none",
  fontFileUrl: "none",
  fill: "none",
  fillRule: "nonzero",
  opacity: 1
}, transform: "translate(165 150) scale(3.4003)" })));
oA(Rt);
const et = "data:image/svg+xml;base64,", Ft = (A, t) => /* @__PURE__ */ Q.createElement("svg", { width: 140, height: 140, xmlns: "http://www.w3.org/2000/svg", ref: t, ...A }, /* @__PURE__ */ Q.createElement("circle", { cy: 60.044, cx: 70, fill: "#5c5c5c", r: 29 }), /* @__PURE__ */ Q.createElement("path", { d: "m16.36 156.186-.01-30.5c-.005-16.844 24.168-30.507 53.991-30.516 29.823-.009 54.004 13.64 54.01 30.484l.008 30.5-108 .032z", fill: "#5c5c5c" }));
oA(Ft);
const zt = "data:image/svg+xml;base64,", Ut = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 0 637 637", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "m478 113-13 2-4 1h4l14-2c10-2 35-2 42-1l6 3 3 1c3 0-4-3-8-5-7-1-34-1-44 1m56 6 8 6c7 5 20 17 20 19s7 12 8 12l-3-6-4-6c0-3-15-17-25-24-5-3-8-4-4-1m-198 16-9 1a512 512 0 0 1 9-1m-180 30a412 412 0 0 0-23 4 167 167 0 0 0-32 8 439 439 0 0 1 46-9 304 304 0 0 1 11-2l6-1h-8m369 8 4 3-7-6c-1 0 0 2 3 3m-439 9c-7 3-18 13-18 15l6-5c5-5 9-8 14-10l6-4-8 4m442 12-10 11-7 6 7-6c7-5 14-14 13-16l-3 5M65 205l-1 11c0 9 0 10 4 17 5 10 7 12 22 11 10 0 15 0 38-5l5-1-6 1-16 2-21 2c-13 1-16-1-21-10-3-5-4-6-4-15l1-12c1-4 1-5-1-1m335 1c3 0 3 1 3 3l-2 6c-2 5-35 39-76 80-53 51-120 117-138 137-21 24-22 25-22 27 0 3 6 9 9 9 4 0 14-8 38-27a5073 5073 0 0 1 15-16 1421 1421 0 0 0-17 15l-31 25-6 2-4-3c-3-3-3-4-2-6 3-8 79-85 160-163 57-57 79-80 79-86 0-2-3-4-7-4-2 0-2 0 1 1m165 18c-9 17-25 32-57 53l-10 8 16-11 12-8c15-10 27-22 39-39 5-7 6-8 4-8l-4 5m-221 2a199 199 0 0 0-19 10 183 183 0 0 1 27-12c0-1-1-1-8 2m-34 16c-3 3-2 3 2 0l1-2-3 2m151 3-1 2 2-2c3-3 2-3-1 0m-16 12-11 10a281 281 0 0 0 11-10m-24 18-7 5-7 5-42 31c-10 7-24 19-22 19l7-6 16-12a343 343 0 0 0 35-25 573 573 0 0 0 20-17m-187 0-2 2 4-2 3-2-5 2m-14 6-3 2 4-1 4-3-5 2m-7 3-8 4-18 8 18-7 7-3 3-1c1-2 0-2-2-1m254 24a2672 2672 0 0 1-71 54 601 601 0 0 0 39-29 1352 1352 0 0 1 32-25m-291-8-2 2 4-2 3-2-5 2m-13 5-2 2h3c5-3 4-4-1-2m-19 9-7 3-13 6c-10 5-13 7-14 10-3 4-2 5 1 0 2-3 5-5 14-9l13-6 12-6-6 2m178 36c-17 12-21 16-21 17a390 390 0 0 1 20-17m-206 34 1 4 1 3 1 5 2 8 1 5v-6l-1-8-2-5-1-4-1-2c-1-2-1-2-1 0m222 23-9 7a683 683 0 0 0-56 44 261 261 0 0 0-26 20 1345 1345 0 0 0-46 34c-6 3-7 3-19 3-11 0-12 0-18-3-8-4-10-8-17-20l-5-10 4 10c5 10 12 18 19 22 4 2 6 2 17 2 13 0 14 0 21-3l26-19 42-33a2183 2183 0 0 0 43-34 643 643 0 0 1 24-20", fill: "#fff", fillRule: "evenodd" }), /* @__PURE__ */ Q.createElement("path", { d: "M478 114a620 620 0 0 1-39 6l-21 3a5481 5481 0 0 0-19 3 870 870 0 0 1-54 8l-1 1-6 1-12 2-13 2a992 992 0 0 0-76 12 400 400 0 0 1-45 8l-6 1-5 1h-3l-3 1-2 1-26 4-13 2-17 3c-23 5-33 9-43 19-7 8-9 13-9 27 0 8 1 9 4 14 5 9 8 11 21 10l21-2a1133 1133 0 0 1 23-4 659 659 0 0 0 74-14l3-1h3l3-1h3l2-1 2-1h3l2-1h2l24-5 38-7a2100 2100 0 0 1 93-18l3-1 10-1a2082 2082 0 0 1 120-17l2 1c2 0 8 5 8 7l1 2 1 6c0 4 0 6-3 10-2 4-16 16-18 16l-7 5-10 8a1070 1070 0 0 0-11 7 113 113 0 0 1-20 14l-2 2-3 2-2 2h-1a154 154 0 0 1-28 21l-5 4-5 4a573 573 0 0 1-50 37 175 175 0 0 0-24 18l-1 2-2 1-2 1-2 1-1 1-12 10a396 396 0 0 0-32 25l-8 6-6 5-9 7a326 326 0 0 1-25 20l-13 12a5073 5073 0 0 0-17 15c-24 20-34 27-39 27-2 0-8-6-8-9 0-2 1-3 22-27 18-20 85-86 138-137 41-41 74-75 76-80l2-6c0-2 0-3-2-3l-7 1a233 233 0 0 1-26 9l-1 1-1 1h-1l-1 1h-2l-3 2-1 1h-2l-3 2h-1l-5 2-22 10-9 4-4 2-5 2-10 4-2 1-2 1-2 1-13 6a334 334 0 0 1-38 17 75 75 0 0 0-14 6l-5 2-4 1-2 1-3 2-7 3-16 6a247 247 0 0 0-32 14l-1 1h-2l-1 1h-2l-1 1-12 6a103 103 0 0 0-27 15l-2 5-1 1v11l1 5 1 5 1 4c0 3 0 4 2 4v3l1 3 1 2 2 10 1 4 1 5 2 10 2 8 1 4 1 4c3 3 9 25 11 38l1 7v2l1 4v1l1 1v2l1 2 1 3v1l5 10c7 13 9 16 17 20 6 3 7 3 18 3 12 0 13 0 19-3 8-4 11-6 26-18a1345 1345 0 0 1 85-67 683 683 0 0 1 50-39l12-9a513 513 0 0 1 44-33 201 201 0 0 0 22-17 2270 2270 0 0 0 73-54 158 158 0 0 0 58-59c2 0 3-9 4-27l-1-19-1-4-1-6-1-3-4-6-4-6c0-2-6-8-13-14a77 77 0 0 0-28-18c-6-1-34-1-43 1" })), ot = oA(Ut), jt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M765-321q0 117-84 196-83 80-200 80-118 0-200-80t-83-196v-392q0-85 61-144 61-58 146-58 86 0 147 58 61 59 61 144v374q0 54-39 92-39 37-93 37-55 0-93-37-38-38-38-92v-372h110v372q0 9 6 14t15 5q8 0 15-5t7-14v-374q0-39-29-66t-68-27q-40 0-69 27t-29 66v392q0 71 51 119t122 48q71 0 122-48t51-119v-426h111v426Z" })), nt = oA(jt), Zt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M125-165v-113h77v-273q0-93 55-167 56-75 146-100v-19q0-32 22-55 23-23 55-23t55 23q22 23 22 55v19q90 25 146 99 56 73 56 168v273h77v113H125ZM480-32q-38 0-66-27-27-27-27-66h186q0 39-27 66t-66 27Z" })), Gt = oA(Zt), Wt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M137-190v-113h78v-253q0-41 11-79 11-39 35-71l-9 150h10L17-802l74-74L873-92l-74 74-175-172H137Zm609-149L319-769q20-15 41-25t44-16v-28q0-32 22-54t54-22q32 0 55 22t22 54v28q85 23 137 94t52 160v217ZM481-47q-38 0-65-27t-27-65h184q0 38-27 65t-65 27Z" })), Yt = oA(Wt), Xt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 100 100", ref: t, ...A }, /* @__PURE__ */ Q.createElement("g", { strokeLinecap: "round", strokeWidth: 10 }, /* @__PURE__ */ Q.createElement("path", { d: "m19 19 61 61M80 19 19 80" }))), Jt = oA(Xt), Vt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M94-136v-255l351-93-351-92v-253l816 346L94-136Z" })), Kt = oA(Vt), qt = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M480-372q-61 0-105-43-43-43-43-105v-240q0-61 43-105 43-43 105-43 61 0 105 43 43 44 43 105v240q0 62-43 105t-105 43ZM425-76v-122q-121-16-197-108t-76-214h109q0 91 64 154t155 63q91 0 155-63t64-154h109q0 122-77 214T534-198v122H425Z" })), _t = oA(qt), $t = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "m763-343-80-79q8-20 13-44 5-23 5-54h109q0 49-11 95t-36 82ZM625-482 336-770v-2q3-56 45-96t101-40q62 0 105 43t43 105v249l-1 17q-1 7-4 12ZM427-76v-122q-120-16-196-108-77-92-77-214h109q0 91 64 154t155 63q43 0 80-14 37-15 67-41l78 79q-35 32-78 53t-93 28v122H427Zm362 27L41-796l75-75 748 748-75 74Z" })), Ae = oA($t), te = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M625-511q29 0 50-21t20-50q0-29-20-50-21-20-50-20t-50 20-20 50q0 29 20 50 21 21 50 21Zm-290 0q29 0 50-21t20-49q0-30-20-50t-50-21q-29 0-50 20-20 21-20 50 0 30 20 50 21 21 50 21Zm145 268q75 0 137-43t85-115H258q24 72 85 115 62 43 137 43Zm0 198q-90 0-170-34-79-34-138-93T79-310q-34-80-34-170t34-170q34-79 93-138t138-93q80-34 170-34t170 34q79 34 138 93t93 138q34 80 34 170t-34 170q-34 79-93 138T650-79q-80 34-170 34Zm0-435Zm0 322q134 0 228-94t94-228q0-134-94-228t-228-94q-134 0-228 94t-94 228q0 134 94 228t228 94Z" })), st = oA(te), ee = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M553-64v-108q107-27 173-113 67-87 67-197 0-109-67-195T553-789v-108q152 26 249 144 97 117 97 271 0 155-97 273T553-64ZM61-335v-290h176l236-237v764L237-335H61Zm492 28v-346q51 23 82 70t31 103-31 103q-31 47-82 70Z" })), oe = oA(ee), ne = (A, t) => /* @__PURE__ */ Q.createElement("svg", { viewBox: "0 -960 960 960", ref: t, ...A }, /* @__PURE__ */ Q.createElement("path", { d: "M807-15 700-123q-33 21-71 37-38 15-80 22v-108q20-4 38-12l35-17-153-154v257L234-335H57v-290h144L10-818l74-75L882-90l-75 75Zm37-264-80-79q13-30 19-60 7-31 7-64 0-109-68-195-67-86-173-112v-108q152 26 249 144 98 117 98 271 0 55-14 107-13 51-38 96ZM661-462 549-575v-78q51 23 83 70 31 47 31 103l-1 9-1 9ZM469-655 366-759l103-103v207Z" })), se = oA(ne), re = "data:audio/wav;base64,", it = {
  general: {
    primaryColor: "#42b0c5",
    secondaryColor: "#491d8d",
    fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif",
    showHeader: !0,
    showFooter: !0,
    showInputRow: !0,
    actionDisabledIcon: xt,
    embedded: !1,
    flowStartTrigger: "ON_LOAD"
  },
  tooltip: {
    mode: "CLOSE",
    text: "Talk to me! 😊"
  },
  chatButton: {
    icon: ot
  },
  header: {
    title: /* @__PURE__ */ l("div", { style: { cursor: "pointer", margin: 0, fontSize: 20, fontWeight: "bold" }, onClick: () => window.open("https://github.com/tjtanjin/"), children: "Tan Jin" }),
    showAvatar: !0,
    avatar: et,
    buttons: [J.NOTIFICATION_BUTTON, J.AUDIO_BUTTON, J.CLOSE_CHAT_BUTTON],
    closeChatIcon: Jt
  },
  notification: {
    disabled: !1,
    defaultToggledOn: !0,
    volume: 0.2,
    icon: Gt,
    iconDisabled: Yt,
    sound: re,
    showCount: !0
  },
  audio: {
    disabled: !0,
    defaultToggledOn: !1,
    language: "en-US",
    voiceNames: ["Microsoft David - English (United States)", "Alex (English - United States)"],
    rate: 1,
    volume: 1,
    icon: oe,
    iconDisabled: se
  },
  chatHistory: {
    disabled: !1,
    maxEntries: 30,
    storageKey: "rcb-history",
    storageType: "LOCAL_STORAGE",
    viewChatHistoryButtonText: "Load Chat History ⟳",
    chatHistoryLineBreakText: "----- Previous Chat History -----",
    autoLoad: !1
  },
  chatInput: {
    disabled: !1,
    allowNewline: !1,
    enabledPlaceholderText: "Type your message...",
    disabledPlaceholderText: "",
    showCharacterCount: !1,
    characterLimit: -1,
    botDelay: 1e3,
    sendButtonIcon: Kt,
    blockSpam: !0,
    sendOptionOutput: !0,
    sendCheckboxOutput: !0,
    buttons: [J.VOICE_MESSAGE_BUTTON, J.SEND_MESSAGE_BUTTON]
  },
  chatWindow: {
    showScrollbar: !1,
    autoJumpToBottom: !1,
    showMessagePrompt: !0,
    messagePromptText: "New Messages ↓",
    messagePromptOffset: 30,
    defaultOpen: !1
  },
  sensitiveInput: {
    maskInTextArea: !0,
    maskInUserBubble: !0,
    asterisksCount: 10,
    hideInUserBubble: !1
  },
  userBubble: {
    animate: !0,
    showAvatar: !1,
    avatar: zt,
    simStream: !1,
    streamSpeed: 30,
    dangerouslySetInnerHtml: !1
  },
  botBubble: {
    animate: !0,
    showAvatar: !1,
    avatar: et,
    simStream: !1,
    streamSpeed: 30,
    dangerouslySetInnerHtml: !1
  },
  voice: {
    disabled: !0,
    defaultToggledOn: !1,
    language: "en-US",
    timeoutPeriod: 1e4,
    autoSendDisabled: !1,
    autoSendPeriod: 1e3,
    sendAsAudio: !1,
    icon: _t,
    iconDisabled: Ae
  },
  footer: {
    text: /* @__PURE__ */ G(
      "div",
      {
        style: { cursor: "pointer", display: "flex", flexDirection: "row", alignItems: "center", columnGap: 3 },
        onClick: () => window.open("https://react-chatbotify.com"),
        children: [
          /* @__PURE__ */ l("span", { children: "Powered By " }, 0),
          /* @__PURE__ */ l(
            "div",
            {
              style: {
                borderRadius: "50%",
                width: 14,
                height: 14,
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
                background: "linear-gradient(to right, #42b0c5, #491d8d)"
              },
              children: /* @__PURE__ */ l(ot, { style: { width: "80%", height: "80%", fill: "#fff" } })
            },
            1
          ),
          /* @__PURE__ */ l("span", { style: { fontWeight: "bold" }, children: " React ChatBotify" }, 2)
        ]
      }
    ),
    buttons: [J.FILE_ATTACHMENT_BUTTON, J.EMOJI_PICKER_BUTTON]
  },
  fileAttachment: {
    disabled: !1,
    multiple: !0,
    accept: ".png",
    icon: nt,
    iconDisabled: nt,
    sendFileName: !0,
    showMediaDisplay: !1
  },
  emoji: {
    disabled: !1,
    icon: st,
    iconDisabled: st,
    list: ["😀", "😃", "😄", "😅", "😊", "😌", "😇", "🙃", "🤣", "😍", "🥰", "🥳", "🎉", "🎈", "🚀", "⭐️"]
  },
  toast: {
    maxCount: 3,
    forbidOnMax: !1,
    dismissOnClick: !0
  },
  event: {
    rcbPreInjectMessage: !1,
    rcbPostInjectMessage: !1,
    rcbStartStreamMessage: !1,
    rcbChunkStreamMessage: !1,
    rcbStopStreamMessage: !1,
    rcbRemoveMessage: !1,
    rcbLoadChatHistory: !1,
    rcbToggleChatWindow: !1,
    rcbToggleAudio: !1,
    rcbToggleNotifications: !1,
    rcbToggleVoice: !1,
    rcbChangePath: !1,
    rcbShowToast: !1,
    rcbDismissToast: !1,
    rcbUserSubmitText: !1,
    rcbUserUploadFile: !1,
    rcbTextAreaChangeValue: !1,
    rcbPreLoadChatBot: !1,
    rcbPostLoadChatBot: !1
  },
  ariaLabel: {
    chatButton: "open chat",
    audioButton: "toggle audio",
    closeChatButton: "close chat",
    emojiButton: "emoji picker",
    fileAttachmentButton: "upload file",
    notificationButton: "toggle notifications",
    sendButton: "send message",
    voiceButton: "toggle voice",
    inputTextArea: "input text area"
  },
  device: {
    desktopEnabled: !0,
    mobileEnabled: !0,
    applyMobileOptimizations: !0
  }
}, lt = bA({ settings: {}, setSettings: () => null }), x = () => hA(lt), ae = ({
  children: A,
  settings: t = it,
  setSettings: e
}) => /* @__PURE__ */ l(lt.Provider, { value: { settings: t, setSettings: e }, children: A }), ut = {
  tooltipStyle: {},
  notificationBadgeStyle: {},
  chatWindowStyle: {},
  headerStyle: {},
  bodyStyle: {},
  chatInputContainerStyle: {},
  chatInputAreaStyle: {},
  chatInputAreaFocusedStyle: {},
  chatInputAreaDisabledStyle: {},
  userBubbleStyle: {},
  botBubbleStyle: {},
  botOptionStyle: {},
  botOptionHoveredStyle: {},
  botCheckboxRowStyle: {},
  botCheckboxNextStyle: {},
  botCheckMarkStyle: {},
  botCheckMarkSelectedStyle: {},
  characterLimitStyle: {},
  characterLimitReachedStyle: {},
  chatHistoryLineBreakStyle: {},
  chatMessagePromptStyle: {},
  chatMessagePromptHoveredStyle: {},
  footerStyle: {},
  loadingSpinnerStyle: {},
  mediaDisplayContainerStyle: {},
  chatButtonStyle: {},
  chatHistoryButtonStyle: {},
  chatHistoryButtonHoveredStyle: {},
  sendButtonStyle: {},
  sendButtonHoveredStyle: {},
  sendButtonDisabledStyle: {},
  audioButtonStyle: {},
  audioButtonDisabledStyle: {},
  closeChatButtonStyle: {},
  emojiButtonStyle: {},
  emojiButtonDisabledStyle: {},
  fileAttachmentButtonStyle: {},
  fileAttachmentButtonDisabledStyle: {},
  notificationButtonStyle: {},
  notificationButtonDisabledStyle: {},
  voiceButtonStyle: {},
  voiceButtonDisabledStyle: {},
  chatIconStyle: {},
  audioIconStyle: {},
  audioIconDisabledStyle: {},
  closeChatIconStyle: {},
  emojiIconStyle: {},
  emojiIconDisabledStyle: {},
  fileAttachmentIconStyle: {},
  fileAttachmentIconDisabledStyle: {},
  notificationIconStyle: {},
  notificationIconDisabledStyle: {},
  voiceIconStyle: {},
  voiceIconDisabledStyle: {},
  sendIconStyle: {},
  sendIconDisabledStyle: {},
  sendIconHoveredStyle: {},
  rcbTypingIndicatorContainerStyle: {},
  rcbTypingIndicatorDotStyle: {},
  toastPromptContainerStyle: {},
  toastPromptStyle: {},
  toastPromptHoveredStyle: {}
}, dt = bA({ styles: {}, setStyles: () => null }), W = () => hA(dt), ce = ({
  children: A,
  styles: t = ut,
  setStyles: e
}) => /* @__PURE__ */ l(dt.Provider, { value: { styles: t, setStyles: e }, children: A });
const ie = ({ buttons: A }) => {
  var n, s, a, r, c;
  const { settings: t } = x(), { styles: e } = W(), o = {
    backgroundImage: `linear-gradient(to right, ${(n = t.general) == null ? void 0 : n.secondaryColor},
			${(s = t.general) == null ? void 0 : s.primaryColor})`,
    ...e.headerStyle
  };
  return /* @__PURE__ */ G("div", { style: o, className: "rcb-chat-header-container", children: [
    /* @__PURE__ */ G("div", { className: "rcb-chat-header", children: [
      ((a = t.header) == null ? void 0 : a.showAvatar) && /* @__PURE__ */ l(
        "div",
        {
          style: { backgroundImage: `url(${(r = t.header) == null ? void 0 : r.avatar})` },
          className: "rcb-bot-avatar"
        }
      ),
      (c = t.header) == null ? void 0 : c.title
    ] }),
    /* @__PURE__ */ l("div", { className: "rcb-chat-header", children: A == null ? void 0 : A.map(
      (i, u) => /* @__PURE__ */ l(ZA, { children: i }, u)
    ) })
  ] });
}, wt = bA({}), q = () => hA(wt), le = ({
  children: A,
  botIdRef: t,
  flowRef: e
}) => {
  const o = tA(null), n = tA(""), s = tA(/* @__PURE__ */ new Map()), a = tA(null), r = tA(""), c = tA(!1), i = tA(null), u = tA(null), d = tA(null);
  return /* @__PURE__ */ l(wt.Provider, { value: {
    botIdRef: t,
    flowRef: e,
    inputRef: o,
    streamMessageMap: s,
    chatBodyRef: a,
    paramsInputRef: r,
    keepVoiceOnRef: c,
    audioContextRef: i,
    audioBufferRef: u,
    gainNodeRef: d,
    prevInputRef: n
  }, children: A });
}, ft = bA({}), AA = () => hA(ft), ue = ({
  children: A,
  settings: t
}) => {
  var Z, F, Y, _, X, V;
  const [e, o] = H(!1), [n, s] = H(((Z = t == null ? void 0 : t.chatWindow) == null ? void 0 : Z.defaultOpen) ?? !1), [a, r] = H(((F = t == null ? void 0 : t.audio) == null ? void 0 : F.defaultToggledOn) ?? !1), [c, i] = H(((Y = t == null ? void 0 : t.voice) == null ? void 0 : Y.defaultToggledOn) ?? !1), [u, d] = H(
    ((_ = t == null ? void 0 : t.notification) == null ? void 0 : _.defaultToggledOn) ?? !0
  ), [h, w] = H(!1), [v, m] = H(!1), [b, g] = H(!0), [D, E] = H(!1), [I, f] = H(!1), [P, B] = H(!1), [y, p] = H(0), [T, M] = H(0), [L, C] = H(!1), [S, N] = H(null), [j, U] = H(((X = window.visualViewport) == null ? void 0 : X.height) ?? window.innerHeight), [R, z] = H(((V = window.visualViewport) == null ? void 0 : V.width) ?? window.innerWidth);
  return /* @__PURE__ */ l(ft.Provider, { value: {
    isBotTyping: e,
    setIsBotTyping: o,
    isChatWindowOpen: n,
    setIsChatWindowOpen: s,
    audioToggledOn: a,
    setAudioToggledOn: r,
    voiceToggledOn: c,
    setVoiceToggledOn: i,
    notificationsToggledOn: u,
    setNotificationsToggledOn: d,
    isLoadingChatHistory: h,
    setIsLoadingChatHistory: w,
    isScrolling: v,
    setIsScrolling: m,
    textAreaDisabled: b,
    setTextAreaDisabled: g,
    textAreaSensitiveMode: D,
    setTextAreaSensitiveMode: E,
    hasInteractedPage: I,
    setHasInteractedPage: f,
    hasFlowStarted: P,
    setHasFlowStarted: B,
    unreadCount: y,
    setUnreadCount: p,
    inputLength: T,
    setInputLength: M,
    blockAllowsAttachment: L,
    setBlockAllowsAttachment: C,
    timeoutId: S,
    setTimeoutId: N,
    viewportHeight: j,
    setViewportHeight: U,
    viewportWidth: R,
    setViewportWidth: z
  }, children: A });
};
const de = () => {
  var v, m, b;
  const { settings: A } = x(), { styles: t } = W(), { unreadCount: e, isScrolling: o, setIsScrolling: n } = AA(), { chatBodyRef: s } = q(), [a, r] = H(!1), c = {
    color: (v = A.general) == null ? void 0 : v.primaryColor,
    borderColor: (m = A.general) == null ? void 0 : m.primaryColor,
    ...t.chatMessagePromptHoveredStyle
  }, i = () => {
    r(!0);
  }, u = () => {
    r(!1);
  };
  function d(g) {
    if (!s.current)
      return;
    const D = s.current.scrollTop, I = s.current.scrollHeight - s.current.clientHeight - D, f = 20;
    let P = 0;
    function B() {
      if (!s.current)
        return;
      P += f;
      const y = h(P, D, I, g);
      s.current.scrollTop = y, P < g ? requestAnimationFrame(B) : n(!1);
    }
    B();
  }
  const h = (g, D, E, I) => (g /= I / 2, g < 1 ? E / 2 * g * g + D : (g--, -E / 2 * (g * (g - 2) - 1) + D));
  return /* @__PURE__ */ l("div", { className: `rcb-message-prompt-container ${(() => {
    var D;
    return s.current && ((D = A.chatWindow) == null ? void 0 : D.showMessagePrompt) && o && e > 0 ? "visible" : "hidden";
  })()}`, children: /* @__PURE__ */ l(
    "div",
    {
      onMouseEnter: i,
      onMouseLeave: u,
      style: a ? c : t.chatMessagePromptStyle,
      onMouseDown: (g) => {
        g.preventDefault(), d(600);
      },
      className: "rcb-message-prompt-text",
      children: (b = A.chatWindow) == null ? void 0 : b.messagePromptText
    }
  ) });
}, k = {
  // audio
  TOGGLE_AUDIO: "rcb-toggle-audio",
  // notifications:
  TOGGLE_NOTIFICATIONS: "rcb-toggle-notifications",
  // voice
  TOGGLE_VOICE: "rcb-toggle-voice",
  // chat window
  TOGGLE_CHAT_WINDOW: "rcb-toggle-chat-window",
  // messages:
  PRE_INJECT_MESSAGE: "rcb-pre-inject-message",
  POST_INJECT_MESSAGE: "rcb-post-inject-message",
  START_STREAM_MESSAGE: "rcb-start-stream-message",
  CHUNK_STREAM_MESSAGE: "rcb-chunk-stream-message",
  STOP_STREAM_MESSAGE: "rcb-stop-stream-message",
  REMOVE_MESSAGE: "rcb-remove-message",
  // chat history
  LOAD_CHAT_HISTORY: "rcb-load-chat-history",
  // path
  CHANGE_PATH: "rcb-change-path",
  // toast
  SHOW_TOAST: "rcb-show-toast",
  DISMISS_TOAST: "rcb-dismiss-toast",
  // user input submission
  USER_SUBMIT_TEXT: "rcb-user-submit-text",
  USER_UPLOAD_FILE: "rcb-user-upload-file",
  // text area value change
  TEXT_AREA_CHANGE_VALUE: "rcb-text-area-change-value",
  // chatbot loading
  PRE_LOAD_CHATBOT: "rcb-pre-load-chatbot",
  POST_LOAD_CHATBOT: "rcb-post-load-chatbot"
}, we = {
  [k.TOGGLE_AUDIO]: !0,
  [k.TOGGLE_VOICE]: !0,
  [k.TOGGLE_NOTIFICATIONS]: !0,
  [k.TOGGLE_CHAT_WINDOW]: !0,
  [k.PRE_INJECT_MESSAGE]: !0,
  [k.POST_INJECT_MESSAGE]: !1,
  [k.PRE_LOAD_CHATBOT]: !0,
  [k.POST_LOAD_CHATBOT]: !1,
  [k.START_STREAM_MESSAGE]: !0,
  [k.CHUNK_STREAM_MESSAGE]: !0,
  [k.STOP_STREAM_MESSAGE]: !0,
  [k.LOAD_CHAT_HISTORY]: !0,
  [k.CHANGE_PATH]: !0,
  [k.SHOW_TOAST]: !0,
  [k.DISMISS_TOAST]: !0,
  [k.USER_SUBMIT_TEXT]: !0,
  [k.USER_UPLOAD_FILE]: !0,
  [k.TEXT_AREA_CHANGE_VALUE]: !0
}, HA = async (A, t, e) => {
  const o = new CustomEvent(A, {
    detail: t,
    cancelable: we[A]
  });
  return o.data = e, o.promises = [], window.dispatchEvent(o), await Promise.all(o.promises), o;
}, gt = bA({ paths: [], setPaths: () => null }), GA = () => hA(gt), fe = ({ children: A }) => {
  const [t, e] = H([]);
  return /* @__PURE__ */ l(gt.Provider, { value: { paths: t, setPaths: e }, children: A });
}, SA = () => {
  const { settings: A } = x(), { paths: t, setPaths: e } = GA(), {
    setIsBotTyping: o,
    setTextAreaDisabled: n,
    setTextAreaSensitiveMode: s,
    blockAllowsAttachment: a,
    setBlockAllowsAttachment: r
  } = AA(), { botIdRef: c } = q(), i = O(() => t.length > 0 ? t[t.length - 1] : null, [t]), u = O(() => t.length > 1 ? t[t.length - 2] : null, [t]), d = O(async (w) => {
    var v, m;
    if ((v = A.event) != null && v.rcbChangePath) {
      const b = i(), g = u(), D = { botId: c.current, currPath: b, prevPath: g };
      if (event = await HA(k.CHANGE_PATH, D, { currPath: b, prevPath: g, nextPath: w }), event.defaultPrevented)
        return !1;
    }
    return o(!0), (m = A.chatInput) != null && m.blockSpam && n(!0), s(!1), e((b) => [...b, w]), !0;
  }, [t, e, A]), h = O((w) => {
    e(w);
  }, []);
  return {
    getCurrPath: i,
    getPrevPath: u,
    goToPath: d,
    blockAllowsAttachment: a,
    setBlockAllowsAttachment: r,
    paths: t,
    replacePaths: h
  };
}, ge = () => {
  const {
    getCurrPath: A,
    getPrevPath: t,
    goToPath: e,
    paths: o,
    replacePaths: n
  } = SA();
  return {
    getCurrPath: A,
    getPrevPath: t,
    goToPath: e,
    paths: o,
    replacePaths: n
  };
}, uA = () => {
  const { getCurrPath: A, getPrevPath: t, paths: e } = ge(), { botIdRef: o } = q();
  return { callRcbEvent: O(async (s, a) => {
    const r = { botId: o.current, currPath: A(), prevPath: t() };
    return await HA(s, r, a);
  }, [e]) };
}, dA = () => {
  const { settings: A } = x(), {
    isChatWindowOpen: t,
    setIsChatWindowOpen: e,
    viewportHeight: o,
    setViewportHeight: n,
    viewportWidth: s,
    setViewportWidth: a,
    setUnreadCount: r
  } = AA(), { callRcbEvent: c } = uA(), [i, u] = H(0), d = O(async () => {
    var w;
    (w = A.event) != null && w.rcbToggleChatWindow && (await c(
      k.TOGGLE_CHAT_WINDOW,
      { currState: t, newState: !t }
    )).defaultPrevented || e((v) => (v || r(0), !v));
  }, [t]), h = O(async (w) => {
    t !== w && await d();
  }, [t]);
  return {
    isChatWindowOpen: t,
    setIsChatWindowOpen: e,
    toggleChatWindow: d,
    openChat: h,
    chatScrollHeight: i,
    setChatScrollHeight: u,
    viewportHeight: o,
    setViewportHeight: n,
    viewportWidth: s,
    setViewportWidth: a
  };
}, Bt = bA({ messages: [], setMessages: () => null }), WA = () => hA(Bt), Be = ({ children: A }) => {
  const [t, e] = H([]);
  return /* @__PURE__ */ l(Bt.Provider, { value: { messages: t, setMessages: e }, children: A });
};
const ve = ({
  setChatScrollHeight: A
}) => {
  var E, I, f, P, B, y, p, T, M, L;
  const { settings: t } = x(), { styles: e } = W(), { messages: o } = WA(), { isChatWindowOpen: n } = dA(), {
    isBotTyping: s,
    isScrolling: a,
    setIsScrolling: r,
    setUnreadCount: c
  } = AA(), { chatBodyRef: i } = q(), u = {
    ...e == null ? void 0 : e.bodyStyle,
    scrollbarWidth: (E = t.chatWindow) != null && E.showScrollbar ? "auto" : "none"
  }, d = {
    backgroundColor: (I = t.general) == null ? void 0 : I.primaryColor,
    color: "#fff",
    maxWidth: (f = t.userBubble) != null && f.showAvatar ? "65%" : "70%",
    ...e.userBubbleStyle
  }, h = (P = t.userBubble) != null && P.animate ? "rcb-user-message-entry" : "", w = {
    backgroundColor: (B = t.general) == null ? void 0 : B.secondaryColor,
    color: "#fff",
    maxWidth: (y = t.botBubble) != null && y.showAvatar ? "65%" : "70%",
    ...e.botBubbleStyle
  }, v = (p = t.botBubble) != null && p.animate ? "rcb-bot-message-entry" : "";
  K(() => {
    i.current && (A(i.current.scrollHeight), a || (i.current.scrollTop = i.current.scrollHeight));
  }, [(T = i.current) == null ? void 0 : T.scrollHeight]);
  const m = () => {
    var U, R;
    if (!i.current)
      return;
    A(i.current.scrollHeight);
    const { scrollTop: C, clientHeight: S, scrollHeight: N } = i.current, j = C + S < N - (((U = t.chatWindow) == null ? void 0 : U.messagePromptOffset) ?? 30);
    r(j), j || (C + S >= N - 1 && (i.current.scrollTop = N - S - 1), (n || (R = t.general) != null && R.embedded) && c(0));
  }, b = (C) => C === 0 ? !0 : o[C].sender !== o[C - 1].sender, g = (C, S) => {
    var R, z, Z, F;
    const N = b(S), j = ((R = t.userBubble) == null ? void 0 : R.showAvatar) && N;
    let U = "rcb-user-message";
    return !N && ((z = t.userBubble) != null && z.showAvatar) && (U += " rcb-user-message-offset"), /* @__PURE__ */ G("div", { className: "rcb-user-message-container", children: [
      typeof C.content == "string" ? (Z = t == null ? void 0 : t.userBubble) != null && Z.dangerouslySetInnerHtml ? /* @__PURE__ */ l(
        "div",
        {
          style: { ...d, display: "inline" },
          className: `${U} ${h}`,
          dangerouslySetInnerHTML: { __html: C.content }
        }
      ) : /* @__PURE__ */ l(
        "div",
        {
          style: d,
          className: `${U} ${h}`,
          children: C.content
        }
      ) : C.content,
      j && /* @__PURE__ */ l(
        "div",
        {
          style: { backgroundImage: `url(${(F = t.userBubble) == null ? void 0 : F.avatar})` },
          className: "rcb-message-user-avatar"
        }
      )
    ] });
  }, D = (C, S) => {
    var R, z, Z, F;
    const N = b(S), j = ((R = t.botBubble) == null ? void 0 : R.showAvatar) && N;
    let U = "rcb-bot-message";
    return !N && ((z = t.botBubble) != null && z.showAvatar) && (U += " rcb-bot-message-offset"), /* @__PURE__ */ G("div", { className: "rcb-bot-message-container", children: [
      j && /* @__PURE__ */ l(
        "div",
        {
          style: { backgroundImage: `url(${(Z = t.botBubble) == null ? void 0 : Z.avatar})` },
          className: "rcb-message-bot-avatar"
        }
      ),
      typeof C.content == "string" ? (F = t == null ? void 0 : t.botBubble) != null && F.dangerouslySetInnerHtml ? /* @__PURE__ */ l(
        "div",
        {
          style: { ...w, display: "inline" },
          className: `${U} ${v}`,
          dangerouslySetInnerHTML: { __html: C.content }
        }
      ) : /* @__PURE__ */ l(
        "div",
        {
          style: w,
          className: `${U} ${v}`,
          children: C.content
        }
      ) : C.content
    ] });
  };
  return /* @__PURE__ */ G(
    "div",
    {
      style: u,
      className: "rcb-chat-body-container",
      ref: i,
      onScroll: m,
      children: [
        o.map((C, S) => C.sender === "system" ? /* @__PURE__ */ l("div", { children: C.content }, S) : /* @__PURE__ */ l("div", { children: C.sender === "user" ? g(C, S) : D(C, S) }, S)),
        s && /* @__PURE__ */ G("div", { className: "rcb-bot-message-container", children: [
          ((M = t.botBubble) == null ? void 0 : M.showAvatar) && /* @__PURE__ */ l(
            "div",
            {
              style: { backgroundImage: `url(${(L = t.botBubble) == null ? void 0 : L.avatar})` },
              className: "rcb-message-bot-avatar"
            }
          ),
          /* @__PURE__ */ l(
            "div",
            {
              onMouseDown: (C) => {
                C.preventDefault();
              },
              className: `rcb-bot-message ${v}`,
              children: /* @__PURE__ */ G("div", { className: "rcb-typing-indicator", style: { ...e == null ? void 0 : e.rcbTypingIndicatorContainerStyle }, children: [
                /* @__PURE__ */ l("span", { className: "rcb-dot", style: { ...e == null ? void 0 : e.rcbTypingIndicatorDotStyle } }),
                /* @__PURE__ */ l("span", { className: "rcb-dot", style: { ...e == null ? void 0 : e.rcbTypingIndicatorDotStyle } }),
                /* @__PURE__ */ l("span", { className: "rcb-dot", style: { ...e == null ? void 0 : e.rcbTypingIndicatorDotStyle } })
              ] })
            }
          )
        ] }),
        /* @__PURE__ */ l(de, {})
      ]
    }
  );
};
const De = ({
  checkboxes: A,
  checkedItems: t,
  path: e
}) => {
  var b, g, D, E, I, f, P, B, y, p, T;
  const { settings: o } = x(), { styles: n } = W(), { paths: s } = GA(), { handleSubmitText: a } = IA(), [r, c] = H(/* @__PURE__ */ new Set()), [i, u] = H(!1), d = {
    cursor: i ? `url(${(b = o.general) == null ? void 0 : b.actionDisabledIcon}), auto` : "pointer",
    color: (g = o.general) == null ? void 0 : g.primaryColor,
    borderColor: (D = o.general) == null ? void 0 : D.primaryColor,
    ...n.botCheckboxRowStyle
  }, h = {
    cursor: i || r.size < A.min ? `url(${(E = o.general) == null ? void 0 : E.actionDisabledIcon}), auto` : "pointer",
    color: (I = o.general) == null ? void 0 : I.primaryColor,
    borderColor: (f = o.general) == null ? void 0 : f.primaryColor,
    ...n.botCheckboxNextStyle
  }, w = {
    cursor: i ? `url(${(P = o.general) == null ? void 0 : P.actionDisabledIcon}), auto` : "pointer",
    color: "transparent",
    ...n.botCheckMarkStyle
  }, v = {
    cursor: i ? `url(${(B = o.general) == null ? void 0 : B.actionDisabledIcon}), auto` : "pointer",
    color: "#fff",
    borderColor: (y = o.general) == null ? void 0 : y.primaryColor,
    backgroundColor: (p = o.general) == null ? void 0 : p.primaryColor,
    ...n.botCheckMarkSelectedStyle
  };
  K(() => {
    s.length > 0 && s[s.length - 1] !== e && u(!A.reusable);
  }, [s]);
  const m = (M) => {
    i || c((L) => {
      const C = new Set(L);
      if (C.has(M))
        t.delete(M), C.delete(M);
      else {
        if (r.size == A.max)
          return L;
        t.add(M), C.add(M);
      }
      return C;
    });
  };
  return /* @__PURE__ */ G("div", { className: `rcb-checkbox-container ${(T = o.botBubble) != null && T.showAvatar ? "rcb-checkbox-offset" : ""}`, children: [
    A.items.map((M) => /* @__PURE__ */ l(
      "div",
      {
        onMouseDown: (L) => {
          L.preventDefault(), m(M);
        },
        style: d,
        className: "rcb-checkbox-row-container",
        children: /* @__PURE__ */ G("div", { className: "rcb-checkbox-row", children: [
          /* @__PURE__ */ l(
            "div",
            {
              style: r.has(M) ? v : w,
              className: "rcb-checkbox-mark"
            }
          ),
          /* @__PURE__ */ l("div", { className: "rcb-checkbox-label", children: M })
        ] })
      },
      M
    )),
    /* @__PURE__ */ l(
      "div",
      {
        style: h,
        className: "rcb-checkbox-next-button",
        onMouseDown: (M) => {
          var S;
          if (M.preventDefault(), i || r.size < A.min)
            return;
          const L = Array.from(t).join(", ");
          u(!A.reusable);
          let C;
          A.sendOutput != null ? C = A.sendOutput : C = ((S = o.chatInput) == null ? void 0 : S.sendCheckboxOutput) ?? !0, a(L, C);
        }
      }
    )
  ] });
}, be = async (A, t, e, o) => {
  const n = t.checkboxes;
  if (!n)
    return;
  let s;
  if (typeof n == "function" ? (s = n(o), s instanceof Promise && (s = await s)) : s = n, Array.isArray(s) && (s = { items: s }), !("items" in s) || s.items.length == 0)
    return;
  s.min == null && (s.min = 1), s.max == null && (s.max = s.items.length), s.min > s.max && (s.min = s.max), s.reusable == null && (s.reusable = !1);
  const r = /* @__PURE__ */ l(De, { checkboxes: s, checkedItems: /* @__PURE__ */ new Set(), path: e });
  await o.injectMessage(r);
}, he = async (A, t) => {
  const e = A.function;
  if (!e)
    return;
  const o = e(t);
  return o instanceof Promise ? await o : o;
}, me = async (A, t) => {
  const e = A.message;
  if (!e)
    return;
  if (typeof e == "string") {
    e.trim() !== "" && await t.injectMessage(e);
    return;
  }
  let o = e(t);
  o instanceof Promise && (o = await o), o && o.trim() !== "" && await t.injectMessage(o);
};
const Pe = ({
  options: A,
  path: t
}) => {
  var v, m, b, g, D, E;
  const { settings: e } = x(), { styles: o } = W(), { paths: n } = GA(), { handleSubmitText: s } = IA(), [a, r] = H([]), [c, i] = H(!1), u = {
    cursor: c ? `url(${(v = e.general) == null ? void 0 : v.actionDisabledIcon}), auto` : "pointer",
    color: (m = e.general) == null ? void 0 : m.primaryColor,
    borderColor: (b = e.general) == null ? void 0 : b.primaryColor,
    backgroundColor: "#fff",
    ...o.botOptionStyle
  }, d = {
    color: "#fff",
    borderColor: (g = e.general) == null ? void 0 : g.primaryColor,
    backgroundColor: (D = e.general) == null ? void 0 : D.primaryColor,
    ...o.botOptionHoveredStyle
  };
  K(() => {
    n.length > 0 && n[n.length - 1] !== t && i(!A.reusable);
  }, [n]);
  const h = (I) => {
    r((f) => {
      const P = [...f];
      return P[I] = !0, P;
    });
  }, w = (I) => {
    r((f) => {
      const P = [...f];
      return P[I] = !1, P;
    });
  };
  return /* @__PURE__ */ l("div", { className: `rcb-options-container ${(E = e.botBubble) != null && E.showAvatar ? "rcb-options-offset" : ""}`, children: A.items.map((I, f) => {
    const P = a[f] && !c;
    return /* @__PURE__ */ l(
      "div",
      {
        className: "rcb-options",
        style: P ? d : u,
        onMouseEnter: () => h(f),
        onMouseLeave: () => w(f),
        onMouseDown: (B) => {
          var p;
          if (B.preventDefault(), c)
            return;
          i(!A.reusable);
          let y;
          A.sendOutput != null ? y = A.sendOutput : y = ((p = e.chatInput) == null ? void 0 : p.sendOptionOutput) ?? !0, s(I, y);
        },
        children: I
      },
      I
    );
  }) });
}, Ee = async (A, t, e, o) => {
  const n = t.options;
  if (!n)
    return;
  let s;
  if (typeof n == "function" ? (s = n(o), s instanceof Promise && (s = await s)) : s = n, Array.isArray(s) && (s = { items: s }), !("items" in s) || s.items.length == 0)
    return;
  s.reusable == null && (s.reusable = !1);
  const a = /* @__PURE__ */ l(Pe, { options: s, path: e });
  await o.injectMessage(a);
}, ye = async (A, t, e) => {
  const o = A.path;
  if (!o)
    return !1;
  if (typeof o == "string")
    return e(o);
  let n = o(t);
  return n instanceof Promise && (n = await n), n ? e(n) : !1;
}, Ce = async (A, t) => {
  const e = A.component;
  if (e) {
    if (typeof e == "function") {
      let o = e(t);
      if (o instanceof Promise && (o = await o), !o)
        return;
      await t.injectMessage(o);
      return;
    }
    await t.injectMessage(e);
  }
}, Ie = async (A, t, e, o, n) => {
  const s = A[t];
  if (!s)
    throw new Error("block is not valid.");
  const a = s.transition;
  let r;
  if (typeof a == "function" ? (r = a(e), r instanceof Promise && (r = await r)) : r = a, typeof r == "number" && (r = { duration: r }), !r || r instanceof Promise || r.duration == null || typeof r.duration != "number")
    return;
  r.interruptable == null && (r.interruptable = !1);
  const c = setTimeout(async () => {
    await Dt(A, t, e, o);
  }, r.duration);
  r.interruptable && n(c);
}, pe = async (A, t, e) => {
  const o = A.chatDisabled;
  if (o == null)
    return;
  let n;
  typeof o == "function" ? (n = o(e), n instanceof Promise && (n = await n)) : n = o, t(n);
}, vt = async (A, t, e) => {
  const o = A.isSensitive;
  if (!o) {
    t(!1);
    return;
  }
  let n;
  typeof o == "function" ? (n = o(e), n instanceof Promise && (n = await n)) : n = o, t(n);
}, Qe = async (A, t, e, o, n, s, a) => {
  const r = A[t];
  if (!r)
    throw new Error("Block is not valid.");
  for (const c of Object.keys(r))
    switch (c) {
      case "message":
        await me(r, e);
        break;
      case "options":
        await Ee(A, r, t, e);
        break;
      case "checkboxes":
        await be(A, r, t, e);
        break;
      case "component":
        await Ce(r, e);
        break;
      case "chatDisabled":
        await pe(r, o, e);
        break;
      case "isSensitive":
        await vt(r, n, e);
        break;
      case "transition":
        await Ie(A, t, e, s, a);
    }
}, Dt = async (A, t, e, o) => {
  const n = A[t];
  if (!n)
    throw new Error("Block is not valid.");
  const s = Object.keys(n);
  for (const a of s)
    a === "function" && await he(n, e);
  return s.includes("path") ? await ye(n, e, o) : !1;
}, Se = (A) => {
  const t = [];
  let e = "", o = !1;
  for (let n = 0; n < A.length; n++) {
    const s = A[n];
    s === "<" ? o ? (t.push(e), e = s) : (o = !0, e = s) : s === ">" ? (e += s, t.push(e), e = "", o = !1) : o ? e += s : t.push(s);
  }
  return e !== "" && t.push(e), t;
}, Te = (A) => typeof window.DOMParser < "u" ? new DOMParser().parseFromString(A, "text/html").body.textContent || "" : A.replace(/<\/?[^>]+(>|$)/g, ""), Me = (A, t, e, o, n) => {
  if (!window.SpeechSynthesisUtterance) {
    console.info("Speech Synthesis API is not supported in this environment.");
    return;
  }
  const s = new window.SpeechSynthesisUtterance();
  s.text = A, s.lang = t, s.rate = o, s.volume = n;
  let a = !1;
  for (const r of e)
    if (window.speechSynthesis.getVoices().find((c) => {
      if (c.name === r) {
        s.voice = c, window.speechSynthesis.speak(s), a = !0;
        return;
      }
    }), a)
      break;
  a || window.speechSynthesis.speak(s);
}, He = (A, t, e, o, n) => {
  var a, r, c, i, u, d;
  if ((a = A.audio) != null && a.disabled || o.sender === "user" || typeof o.content != "string" || !e && !((r = A.general) != null && r.embedded) || !t || o.content.trim() === "")
    return;
  let s = o.content;
  n && (s = Te(o.content)), Me(
    s,
    (c = A.audio) == null ? void 0 : c.language,
    (i = A.audio) == null ? void 0 : i.voiceNames,
    (u = A.audio) == null ? void 0 : u.rate,
    (d = A.audio) == null ? void 0 : d.volume
  );
};
const Oe = () => {
  var e;
  const { settings: A } = x(), { styles: t } = W();
  return /* @__PURE__ */ l("div", { className: "rcb-line-break-container", children: /* @__PURE__ */ l(
    "div",
    {
      style: t.chatHistoryLineBreakStyle,
      className: "rcb-line-break-text",
      "data-testid": "chat-history-line-break-text",
      children: (e = A.chatHistory) == null ? void 0 : e.chatHistoryLineBreakText
    }
  ) });
};
const Ne = () => {
  var o;
  const { settings: A } = x(), { styles: t } = W(), e = {
    borderTop: `4px solid ${(o = A.general) == null ? void 0 : o.primaryColor}`,
    ...t.loadingSpinnerStyle
  };
  return /* @__PURE__ */ l("div", { className: "rcb-spinner-container", children: /* @__PURE__ */ l("div", { style: e, className: "rcb-spinner" }) });
}, OA = () => {
  try {
    if (typeof crypto < "u" && typeof crypto.randomUUID == "function")
      return crypto.randomUUID();
    throw new Error("crypto.randomUUID not available");
  } catch {
    if (typeof crypto < "u" && typeof crypto.getRandomValues == "function")
      return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(t) {
        const e = crypto.getRandomValues(new Uint8Array(1))[0] % 16;
        return (t === "x" ? e : e & 3 | 8).toString(16);
      });
    throw new Error("crypto.getRandomValues is also not available");
  }
}, vA = (A, t) => ({
  id: OA(),
  content: A,
  sender: t,
  type: typeof A == "string" ? "string" : "object",
  timestamp: (/* @__PURE__ */ new Date()).toUTCString()
});
let gA, bt = !1, NA = "rcb-history", TA = 30, ht = !1, kA = [];
const FA = async (A) => {
  if (ht || !gA)
    return;
  const t = [], e = bt ? kA.length : 0;
  for (let n = A.length - 1; n >= e; n--) {
    const s = A[n];
    if (s.sender === "system" || (s.content !== "" && t.unshift(s), t.length === TA))
      break;
  }
  let o = t.map(Re);
  if (o.length < TA) {
    const n = TA - o.length;
    o = [...kA.slice(-n), ...o];
  }
  mt(o);
}, ke = (A) => {
  if (A != null)
    try {
      return JSON.parse(A);
    } catch {
      return [];
    }
  return [];
}, YA = () => kA, mt = (A) => {
  gA && gA.setItem(NA, JSON.stringify(A));
}, Le = () => {
  gA && gA.removeItem(NA);
}, xe = (A) => {
  var t, e, o, n, s;
  ((e = (t = A.chatHistory) == null ? void 0 : t.storageType) == null ? void 0 : e.toUpperCase()) === "SESSION_STORAGE" ? gA = sessionStorage : gA = localStorage, NA = (o = A.chatHistory) == null ? void 0 : o.storageKey, TA = (n = A.chatHistory) == null ? void 0 : n.maxEntries, ht = (s = A.chatHistory) == null ? void 0 : s.disabled, kA = ke(gA.getItem(NA));
}, Re = (A) => ct(A.content) ? structuredClone({
  id: A.id,
  content: kt.renderToString(A.content),
  type: A.type,
  sender: A.sender,
  timestamp: A.timestamp
}) : A, Fe = (A, t, e, o, n, s, a) => {
  var r;
  if (bt = !0, e != null)
    try {
      o((i) => {
        const u = vA(/* @__PURE__ */ l(Ne, {}), "system");
        return i.shift(), [u, ...i];
      });
      const c = e.map((i) => {
        if (i.type === "object") {
          const u = Pt(i.content, A, t);
          return { ...i, content: u };
        }
        return i;
      });
      setTimeout(() => {
        o((i) => {
          var d;
          i.shift();
          let u;
          return (d = A.chatHistory) != null && d.autoLoad ? u = vA(/* @__PURE__ */ l(sA, {}), "system") : u = vA(/* @__PURE__ */ l(Oe, {}), "system"), [...c, u, ...i];
        });
      }, 500), setTimeout(() => {
        if (!n.current)
          return;
        const { scrollHeight: i } = n.current, u = i - s;
        n.current.scrollTop = n.current.scrollTop + u, a(!1);
      }, 510);
    } catch {
      gA.removeItem((r = A.chatHistory) == null ? void 0 : r.storageKey);
    }
}, Pt = (A, t, e) => {
  const n = new DOMParser().parseFromString(A, "text/html");
  return Array.from(n.body.childNodes).map((r, c) => {
    var i;
    if (r.nodeType === Node.TEXT_NODE)
      return r.textContent;
    {
      const u = r.tagName.toLowerCase();
      let d = Array.from(r.attributes).reduce((w, v) => {
        const m = v.name.toLowerCase();
        if (m === "style") {
          const b = v.value.split(";").filter((D) => D.trim() !== ""), g = {};
          b.forEach((D) => {
            const [E, I] = D.split(":").map((P) => P.trim()), f = E.replace(/-([a-z])/g, (P, B) => B.toUpperCase());
            g[f] = I;
          }), w[m] = g;
        } else
          (u === "audio" || u === "video") && m === "controls" && v.value === "" ? w[m] = "true" : w[m] = v.value;
        return w;
      }, {});
      if (Object.prototype.hasOwnProperty.call(d, "class")) {
        const w = r.classList;
        d.className = w.toString(), delete d.class, (i = t.botBubble) != null && i.showAvatar && (d = ze(w, d)), d = Ue(w, d, t, e), d = je(w, d, t, e), d = Ze(w, d, t, e), d = Ge(w, d, t, e);
      }
      if ([
        "area",
        "base",
        "br",
        "col",
        "embed",
        "hr",
        "img",
        "input",
        "link",
        "meta",
        "source",
        "track",
        "wbr"
      ].includes(u))
        return tt(u, { key: c, ...d });
      {
        const w = Pt(r.innerHTML, t, e);
        return tt(u, { key: c, ...d }, ...w);
      }
    }
  });
}, ze = (A, t) => ((A.contains("rcb-options-container") || A.contains("rcb-checkbox-container")) && (t.className = `${A.toString()} rcb-options-offset`), t), Ue = (A, t, e, o) => {
  var n, s, a, r, c;
  return A.contains("rcb-options") && (t.style = {
    ...t.style,
    color: ((n = o.botOptionStyle) == null ? void 0 : n.color) ?? ((s = e.general) == null ? void 0 : s.primaryColor),
    borderColor: ((a = o.botOptionStyle) == null ? void 0 : a.color) ?? ((r = e.general) == null ? void 0 : r.primaryColor),
    cursor: `url(${(c = e.general) == null ? void 0 : c.actionDisabledIcon}), auto`,
    ...o.botOptionStyle
  }), t;
}, je = (A, t, e, o) => {
  var n, s, a, r, c;
  return A.contains("rcb-checkbox-row-container") && (t.style = {
    ...t.style,
    color: ((n = o.botCheckboxRowStyle) == null ? void 0 : n.color) ?? ((s = e.general) == null ? void 0 : s.primaryColor),
    borderColor: ((a = o.botCheckboxRowStyle) == null ? void 0 : a.color) ?? ((r = e.general) == null ? void 0 : r.primaryColor),
    cursor: `url(${(c = e.general) == null ? void 0 : c.actionDisabledIcon}), auto`,
    ...o.botCheckboxRowStyle
  }), t;
}, Ze = (A, t, e, o) => {
  var n, s, a, r, c;
  return A.contains("rcb-checkbox-next-button") && (t.style = {
    ...t.style,
    color: ((n = o.botCheckboxNextStyle) == null ? void 0 : n.color) ?? ((s = e.general) == null ? void 0 : s.primaryColor),
    borderColor: ((a = o.botCheckboxNextStyle) == null ? void 0 : a.color) ?? ((r = e.general) == null ? void 0 : r.primaryColor),
    cursor: `url(${(c = e.general) == null ? void 0 : c.actionDisabledIcon}), auto`,
    ...o.botCheckboxNextStyle
  }), t;
}, Ge = (A, t, e, o) => {
  var n, s;
  return (A.contains("rcb-media-display-image-container") || A.contains("rcb-media-display-video-container")) && (t.style = {
    ...t.style,
    backgroundColor: (n = e.general) == null ? void 0 : n.primaryColor,
    maxWidth: (s = e.userBubble) != null && s.showAvatar ? "65%" : "70%",
    ...o.mediaDisplayContainerStyle
  }), t;
}, Et = (A) => {
  if (!A)
    return !1;
  const t = A.getBoundingClientRect(), e = window.innerHeight ?? document.documentElement.clientHeight, o = window.innerWidth ?? document.documentElement.clientWidth;
  return t.top >= 0 && t.left >= 0 && t.bottom <= e && t.right <= o;
}, XA = () => {
  const { settings: A } = x(), {
    notificationsToggledOn: t,
    setNotificationsToggledOn: e,
    hasInteractedPage: o,
    unreadCount: n,
    setUnreadCount: s
  } = AA(), { audioBufferRef: a, audioContextRef: r, gainNodeRef: c } = q(), { callRcbEvent: i } = uA(), u = O(async () => {
    var b, g;
    const w = (b = A.notification) == null ? void 0 : b.sound;
    r.current = new AudioContext();
    const v = r.current.createGain();
    v.gain.value = ((g = A.notification) == null ? void 0 : g.volume) ?? 0.2, c.current = v;
    let m;
    if (w != null && w.startsWith("data:audio")) {
      const D = atob(w.split(",")[1]), E = new ArrayBuffer(D.length), I = new Uint8Array(E);
      for (let f = 0; f < D.length; f++)
        I[f] = D.charCodeAt(f);
      m = E;
    } else
      m = await (await fetch(w)).arrayBuffer();
    a.current = await r.current.decodeAudioData(m);
  }, [A.notification]), d = O(() => {
    var v;
    if ((v = A.notification) != null && v.disabled || !t || !o || !r.current || !a.current)
      return;
    const w = r.current.createBufferSource();
    w.buffer = a.current, w.connect(c.current).connect(r.current.destination), w.start();
  }, [
    A.notification,
    t,
    o,
    r,
    a,
    c
  ]), h = O(async () => {
    var w;
    (w = A.event) != null && w.rcbToggleNotifications && (await i(
      k.TOGGLE_NOTIFICATIONS,
      { currState: t, newState: !t }
    )).defaultPrevented || e((v) => !v);
  }, [t]);
  return {
    unreadCount: n,
    setUnreadCount: s,
    notificationsToggledOn: t,
    toggleNotifications: h,
    playNotificationSound: d,
    setUpNotifications: u
  };
}, yA = () => {
  var E, I, f, P;
  const { settings: A } = x(), { messages: t, setMessages: e } = WA(), {
    audioToggledOn: o,
    isChatWindowOpen: n,
    isScrolling: s,
    setIsBotTyping: a,
    setUnreadCount: r
  } = AA(), { streamMessageMap: c, chatBodyRef: i } = q(), { callRcbEvent: u } = uA(), { playNotificationSound: d } = XA(), h = O(async (B, y, p) => {
    a(!1), c.current.has(B.sender) || (e((S) => {
      const N = [...S, vA("", B.sender)];
      return g(N), N;
    }), c.current.set(B.sender, B.id));
    let T = B.content;
    p && (T = Se(T));
    let M = 0;
    const L = T.length;
    B.content = "", await new Promise((S) => {
      const N = setInterval(() => {
        if (M >= L) {
          clearInterval(N), S();
          return;
        }
        e((j) => {
          const U = [...j];
          for (let R = U.length - 1; R >= 0; R--)
            if (U[R].sender === B.sender && typeof U[R].content == "string") {
              const z = T[M];
              z && (B.content += z, U[R] = B), M++;
              break;
            }
          return U;
        });
      }, y);
    }), c.current.delete(B.sender), FA(t);
  }, [t, c]), w = O(async (B, y = "bot") => {
    var C, S, N, j, U, R, z, Z;
    let p = vA(B, y);
    if ((C = A.event) != null && C.rcbPreInjectMessage) {
      const F = await u(k.PRE_INJECT_MESSAGE, { message: p });
      if (F.defaultPrevented)
        return null;
      p = F.data.message;
    }
    let T = !1;
    y === "bot" ? T = (S = A.botBubble) == null ? void 0 : S.dangerouslySetInnerHtml : y === "user" && (T = (N = A.userBubble) == null ? void 0 : N.dangerouslySetInnerHtml), He(A, o, n, p, T);
    const M = typeof p.content == "string" && p.sender === "bot" && ((j = A == null ? void 0 : A.botBubble) == null ? void 0 : j.simStream), L = typeof p.content == "string" && p.sender === "user" && ((U = A == null ? void 0 : A.userBubble) == null ? void 0 : U.simStream);
    if (r((F) => F + 1), (R = A.event) != null && R.rcbPostInjectMessage && await u(k.POST_INJECT_MESSAGE, { message: p }), M) {
      const F = (z = A.botBubble) == null ? void 0 : z.streamSpeed;
      await h(p, F, T);
    } else if (L) {
      const F = (Z = A.userBubble) == null ? void 0 : Z.streamSpeed;
      await h(p, F, T);
    } else
      e((F) => {
        const Y = [...F, p];
        return g(Y), Y;
      });
    return p.id;
  }, [A, o, n, u, h]), v = O(async (B) => {
    var p;
    const y = t.find((T) => T.id === B);
    return !y || (p = A.event) != null && p.rcbRemoveMessage && (await u(k.REMOVE_MESSAGE, { message: y })).defaultPrevented ? null : (e((T) => {
      const M = T.filter((L) => L.id !== B);
      return g(M), M;
    }), r((T) => Math.max(T - 1, 0)), B);
  }, [u, t, (E = A.event) == null ? void 0 : E.rcbRemoveMessage]), m = O(async (B, y = "bot") => {
    var T, M;
    if (!c.current.has(y)) {
      const L = vA(B, y);
      return (T = A.event) != null && T.rcbStartStreamMessage && (await u(k.START_STREAM_MESSAGE, { message: L })).defaultPrevented ? null : (a(!1), e((C) => {
        const S = [...C, L];
        return g(S), [...C, L];
      }), r((C) => C + 1), c.current.set(y, L.id), L.id);
    }
    const p = { ...vA(B, y), id: c.current.get(y) };
    return (M = A.event) != null && M.rcbChunkStreamMessage && (await u(
      k.CHUNK_STREAM_MESSAGE,
      { ...p, id: c.current.get(y) }
    )).defaultPrevented ? null : (e((L) => {
      const C = [...L];
      for (let S = C.length - 1; S >= 0; S--)
        if (C[S].sender === y && typeof C[S].content == typeof B) {
          C[S] = p;
          break;
        }
      return g(C), C;
    }), c.current.get(y) ?? null);
  }, [u, (I = A.event) == null ? void 0 : I.rcbChunkStreamMessage, (f = A.event) == null ? void 0 : f.rcbStartStreamMessage, c]), b = O(async (B = "bot") => {
    var T;
    if (!c.current.has(B))
      return !0;
    const y = c.current.get(B), p = t.find((M) => M.id === y);
    return (T = A.event) != null && T.rcbStopStreamMessage && (await u(k.STOP_STREAM_MESSAGE, { messageToEndStreamFor: p })).defaultPrevented ? !1 : (c.current.delete(B), FA(t), !0);
  }, [u, t, (P = A.event) == null ? void 0 : P.rcbStopStreamMessage, c]), g = (B) => {
    var T, M;
    FA(B);
    let y = !0;
    (B.length === 0 || n && !s) && (y = !1), (T = A.general) != null && T.embedded && Et(i.current) && (y = !1);
    const p = B[B.length - 1];
    (!p || p.sender === "user") && (y = !1), y && d(), ((M = A.chatWindow) != null && M.autoJumpToBottom || !s) && setTimeout(() => {
      i.current && (i.current.scrollTop = i.current.scrollHeight);
    }, 1);
  }, D = O((B) => {
    g(B), e(B);
  }, [g]);
  return {
    endStreamMessage: b,
    injectMessage: w,
    removeMessage: v,
    streamMessage: m,
    messages: t,
    replaceMessages: D
  };
};
let zA, iA, MA, DA = !1, cA = null;
const JA = () => {
  if (!zA) {
    const A = window.SpeechRecognition || window.webkitSpeechRecognition;
    zA = A != null ? new A() : null;
  }
  return zA;
}, We = (A, t, e, o, n, s, a) => {
  var r;
  (r = A.voice) != null && r.sendAsAudio ? Xe(e, s) : Ye(
    A,
    t,
    e,
    o,
    n,
    a
  );
}, Ye = (A, t, e, o, n, s) => {
  var i, u, d;
  const a = JA();
  if (!a)
    return;
  if (!DA)
    try {
      DA = !0, a.lang = (i = A.voice) == null ? void 0 : i.language, a.start();
    } catch {
    }
  const r = (u = A.voice) == null ? void 0 : u.timeoutPeriod, c = (d = A.voice) == null ? void 0 : d.autoSendPeriod;
  a.onresult = (h) => {
    var v, m;
    clearTimeout(iA), iA = null, clearTimeout(MA);
    const w = h.results[h.results.length - 1][0].transcript;
    if (s.current) {
      const b = (v = A.chatInput) == null ? void 0 : v.characterLimit, g = s.current.value + w;
      b != null && b >= 0 && g.length > b ? o(g.slice(0, b)) : o(g), n(s.current.value.length);
    }
    iA = setTimeout(async () => await UA(t, s), r), (m = A.voice) != null && m.autoSendDisabled || (MA = setTimeout(e, c));
  }, a.onend = () => {
    DA ? (a.start(), iA || (iA = setTimeout(async () => await UA(t, s), r))) : (clearTimeout(iA), iA = null, clearTimeout(MA));
  }, iA = setTimeout(async () => await UA(t, s), r);
}, Xe = (A, t) => {
  navigator.mediaDevices.getUserMedia({ audio: !0 }).then((e) => {
    if (cA = new MediaRecorder(e), !DA)
      try {
        DA = !0, cA.start();
      } catch {
      }
    cA.ondataavailable = (o) => {
      t.current && t.current.push(o.data);
    }, cA.onstop = () => {
      A(), e.getTracks().forEach((o) => o.stop());
    };
  }).catch((e) => {
    console.error("Unable to use microphone:", e);
  });
}, VA = () => {
  const A = JA();
  A && (DA = !1, A && A.stop(), cA && cA.state !== "inactive" && (cA.stop(), cA = null), clearTimeout(iA), iA = null, clearTimeout(MA));
}, Je = (A, t) => {
  var o, n, s;
  const e = JA();
  (o = t.voice) != null && o.disabled || !((n = t.chatInput) != null && n.blockSpam) || !e || (A && !DA ? (DA = !0, (s = t.voice) != null && s.sendAsAudio ? cA == null || cA.start() : e.start()) : A || VA());
}, UA = async (A, t) => {
  var e;
  (e = t.current) != null && e.disabled || await A(), VA();
}, LA = () => {
  const { settings: A } = x(), { voiceToggledOn: t, setVoiceToggledOn: e } = AA(), { callRcbEvent: o } = uA(), n = O(async () => {
    var a;
    (a = A.event) != null && a.rcbToggleVoice && (await o(
      k.TOGGLE_VOICE,
      { currState: t, newState: !t }
    )).defaultPrevented || e((r) => !r);
  }, [t]), s = O((a) => {
    Je(a, A);
  }, [A]);
  return {
    voiceToggledOn: t,
    toggleVoice: n,
    syncVoice: s
  };
}, EA = () => {
  const { settings: A } = x(), {
    inputLength: t,
    setInputLength: e,
    textAreaDisabled: o,
    setTextAreaDisabled: n,
    textAreaSensitiveMode: s,
    setTextAreaSensitiveMode: a
  } = AA(), { inputRef: r, chatBodyRef: c, prevInputRef: i } = q(), { callRcbEvent: u } = uA(), d = O(async (g) => {
    var D, E, I;
    if (o && r.current) {
      r.current.value = "";
      return;
    }
    if (r.current && i.current !== null) {
      const f = (D = A.chatInput) == null ? void 0 : D.characterLimit, B = ((E = A.chatInput) == null ? void 0 : E.allowNewline) ? g : g.replace(/\n/g, " ");
      if (f != null && f >= 0 && B.length > f ? r.current.value = B.slice(0, f) : r.current.value = B, (I = A.event) != null && I.rcbTextAreaChangeValue && (await u(
        k.TEXT_AREA_CHANGE_VALUE,
        { currValue: r.current.value, prevValue: i.current }
      )).defaultPrevented) {
        r.current.value = i.current;
        return;
      }
      i.current = r.current.value;
    }
  }, [o, r, i, A, u]), h = O((g) => {
    var D;
    (D = r.current) != null && D.disabled || setTimeout(() => {
      var E, I, f;
      (E = A.general) != null && E.embedded ? Et(c == null ? void 0 : c.current) && ((I = r.current) == null || I.focus()) : g !== "start" && ((f = r.current) == null || f.focus());
    }, 100);
  }, []), w = O(() => {
    !o && r.current && r.current.focus();
  }, [o]), v = O(() => r && r.current ? r.current.value : "", []), m = O(() => {
    n((g) => !g);
  }, []), b = O(() => {
    a((g) => !g);
  }, []);
  return {
    textAreaDisabled: o,
    setTextAreaDisabled: n,
    textAreaSensitiveMode: s,
    setTextAreaSensitiveMode: a,
    inputLength: t,
    setInputLength: e,
    getTextAreaValue: v,
    setTextAreaValue: d,
    updateTextAreaFocus: h,
    focusTextArea: w,
    toggleTextAreaDisabled: m,
    toggleTextAreaSensitiveMode: b
  };
}, yt = bA({ toasts: [], setToasts: () => null }), Ct = () => hA(yt), Ve = ({ children: A }) => {
  const [t, e] = H([]);
  return /* @__PURE__ */ l(yt.Provider, { value: { toasts: t, setToasts: e }, children: A });
}, CA = () => {
  const { settings: A } = x(), { toasts: t, setToasts: e } = Ct(), { callRcbEvent: o } = uA(), n = tA(t);
  K(() => {
    n.current = t;
  }, [t]);
  const s = O(async (c, i) => {
    var v, m, b, g;
    let u = null;
    if (n.current.length >= (((v = A.toast) == null ? void 0 : v.maxCount) ?? 3)) {
      if ((m = A.toast) != null && m.forbidOnMax)
        return null;
      u = OA();
      let D = { id: u, content: c, timeout: i };
      if ((b = A.event) != null && b.rcbShowToast) {
        const E = await o(k.SHOW_TOAST, { toast: D });
        if (E.defaultPrevented)
          return null;
        D = E.data.toast;
      }
      return e((E) => [...E.slice(1), D]), u;
    }
    u = OA();
    let w = { id: u, content: c, timeout: i };
    if ((g = A.event) != null && g.rcbShowToast) {
      const D = await o(k.SHOW_TOAST, { toast: w });
      if (D.defaultPrevented)
        return null;
      w = D.data.toast;
    }
    return e((D) => [...D, w]), u;
  }, [A, o, e]), a = O(async (c) => {
    var u;
    const i = t.find((d) => d.id === c);
    return !i || (u = A.event) != null && u.rcbDismissToast && (await o(k.DISMISS_TOAST, { toast: i })).defaultPrevented ? null : (e((d) => d.filter((h) => h.id !== c)), c);
  }, [o, e]), r = O((c) => {
    e(c);
  }, []);
  return {
    showToast: s,
    dismissToast: a,
    toasts: t,
    replaceToasts: r
  };
}, IA = () => {
  var S, N, j, U;
  const { settings: A } = x(), { endStreamMessage: t, injectMessage: e, removeMessage: o, streamMessage: n } = yA(), { getCurrPath: s, getPrevPath: a, goToPath: r } = SA(), {
    timeoutId: c,
    voiceToggledOn: i,
    setTextAreaSensitiveMode: u,
    textAreaSensitiveMode: d,
    setTextAreaDisabled: h,
    setIsBotTyping: w,
    setBlockAllowsAttachment: v,
    setInputLength: m
  } = AA(), { flowRef: b, chatBodyRef: g, inputRef: D, keepVoiceOnRef: E, paramsInputRef: I } = q(), { showToast: f, dismissToast: P } = CA(), { callRcbEvent: B } = uA(), { syncVoice: y } = LA(), { setTextAreaValue: p } = EA(), { openChat: T } = dA(), M = O(async (R) => {
    var F, Y, _;
    const z = s();
    if (!(!z || !b.current[z])) {
      if (d) {
        if ((F = A == null ? void 0 : A.sensitiveInput) != null && F.hideInUserBubble)
          return;
        if ((Y = A == null ? void 0 : A.sensitiveInput) != null && Y.maskInUserBubble) {
          await e("*".repeat(((_ = A.sensitiveInput) == null ? void 0 : _.asterisksCount) ?? 10), "user");
          return;
        }
      }
      await e(R, "user");
    }
  }, [b, s, A, e, d]), L = O(async (R, z, Z = !0) => {
    var F, Y;
    z = z.trim(), z !== "" && (Z && await M(z), c && clearTimeout(c), I.current = z, g.current && (g.current.scrollTop = g.current.scrollHeight), D.current && (p(""), m(0)), (F = A.chatInput) != null && F.blockSpam && h(!0), E.current = i, y(!1), setTimeout(() => {
      w(!0);
    }, 400), u(!1), setTimeout(async () => {
      var V;
      const _ = {
        prevPath: a(),
        currPath: s(),
        goToPath: r,
        setTextAreaValue: p,
        userInput: z,
        injectMessage: e,
        streamMessage: n,
        removeMessage: o,
        endStreamMessage: t,
        openChat: T,
        showToast: f,
        dismissToast: P
      };
      if (!await Dt(b.current, R, _, r)) {
        const pA = s();
        if (!pA)
          return;
        const mA = b.current[pA];
        if (!mA)
          return;
        mA.chatDisabled || h((V = A.chatInput) == null ? void 0 : V.disabled), vt(mA, u, _), v(typeof mA.file == "function"), y(E.current), w(!1);
      }
    }, (Y = A.chatInput) == null ? void 0 : Y.botDelay));
  }, [
    c,
    (S = A.chatInput) == null ? void 0 : S.blockSpam,
    (N = A.chatInput) == null ? void 0 : N.botDelay,
    (j = A.chatInput) == null ? void 0 : j.disabled,
    E,
    i,
    y,
    M,
    a,
    s,
    r,
    e,
    n,
    o,
    t,
    T,
    f,
    P,
    b
  ]);
  return { handleSubmitText: O(async (R, z = !0) => {
    var F, Y;
    if (R = R ?? ((F = D.current) == null ? void 0 : F.value), (Y = A.event) != null && Y.rcbUserSubmitText && (await B(k.USER_SUBMIT_TEXT, { inputText: R, sendInChat: z })).defaultPrevented)
      return;
    const Z = s();
    Z && L(Z, R, z);
  }, [B, s, L, D, (U = A.event) == null ? void 0 : U.rcbUserSubmitText]) };
}, xA = () => {
  var e;
  const { settings: A } = x();
  return fA(() => {
    var a;
    if (typeof window > "u" || !window.navigator)
      return !1;
    if (!((a = A.device) != null && a.applyMobileOptimizations))
      return !0;
    const o = navigator.userAgent, n = !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(o), s = window.innerWidth >= 768;
    return n && s;
  }, [(e = A.device) == null ? void 0 : e.applyMobileOptimizations]);
};
const Ke = ({ buttons: A }) => {
  var C, S, N, j, U, R, z, Z, F, Y, _;
  const t = xA(), { settings: e } = x(), { styles: o } = W(), {
    textAreaDisabled: n,
    textAreaSensitiveMode: s,
    inputLength: a,
    hasFlowStarted: r,
    setHasFlowStarted: c,
    setInputLength: i
  } = AA(), { inputRef: u } = q(), [d, h] = H(!1), [w, v] = H(!1), { handleSubmitText: m } = IA(), { setTextAreaValue: b } = EA(), g = {
    boxSizing: t ? "content-box" : "border-box",
    ...o.chatInputAreaStyle
  }, D = {
    outline: n ? "" : "none",
    boxShadow: n ? "" : `0 0 5px ${(C = e.general) == null ? void 0 : C.primaryColor}`,
    boxSizing: t ? "content-box" : "border-box",
    ...o.chatInputAreaStyle,
    // by default inherit the base style for input area
    ...o.chatInputAreaFocusedStyle
  }, E = {
    cursor: `url(${(S = e.general) == null ? void 0 : S.actionDisabledIcon}), auto`,
    caretColor: "transparent",
    boxSizing: t ? "content-box" : "border-box",
    ...o.chatInputAreaStyle,
    // by default inherit the base style for input area
    ...o.chatInputAreaDisabledStyle
  }, I = {
    color: "#989898",
    ...o.characterLimitStyle
  }, f = {
    color: "#ff0000",
    ...o.characterLimitReachedStyle
  }, P = n ? (N = e.chatInput) == null ? void 0 : N.disabledPlaceholderText : (j = e.chatInput) == null ? void 0 : j.enabledPlaceholderText, B = () => {
    n || h(!0);
  }, y = () => {
    h(!1);
  }, p = () => {
    v(!0);
  }, T = () => {
    v(!1);
  }, M = async (X) => {
    var V;
    if (!w && X.key === "Enter") {
      if (X.shiftKey) {
        (V = e.chatInput) != null && V.allowNewline || X.preventDefault();
        return;
      }
      X.preventDefault(), await m();
    }
  }, L = (X) => {
    u.current && (b(X.target.value), i(u.current.value.length));
  };
  return /* @__PURE__ */ G(
    "div",
    {
      "aria-label": ((U = e.ariaLabel) == null ? void 0 : U.inputTextArea) ?? "input text area",
      role: "textbox",
      onMouseDown: (X) => {
        var V;
        X.stopPropagation(), !r && ((V = e.general) == null ? void 0 : V.flowStartTrigger) === "ON_CHATBOT_INTERACT" && c(!0);
      },
      style: o.chatInputContainerStyle,
      className: "rcb-chat-input",
      children: [
        s && ((R = e.sensitiveInput) != null && R.maskInTextArea) ? /* @__PURE__ */ l(
          "input",
          {
            ref: u,
            type: "password",
            className: "rcb-chat-input-textarea",
            style: n ? E : d ? D : g,
            placeholder: P,
            onChange: L,
            onKeyDown: M,
            onFocus: B,
            onBlur: y,
            onCompositionStart: p,
            onCompositionEnd: T
          }
        ) : /* @__PURE__ */ l(
          "textarea",
          {
            ref: u,
            style: n ? E : d ? D : g,
            rows: 1,
            className: "rcb-chat-input-textarea",
            placeholder: P,
            onChange: L,
            onKeyDown: M,
            onFocus: B,
            onBlur: y,
            onCompositionStart: p,
            onCompositionEnd: T
          }
        ),
        /* @__PURE__ */ G("div", { className: "rcb-chat-input-button-container", children: [
          A == null ? void 0 : A.map(
            (X, V) => /* @__PURE__ */ l(ZA, { children: X }, V)
          ),
          ((z = e.chatInput) == null ? void 0 : z.showCharacterCount) && ((Z = e.chatInput) == null ? void 0 : Z.characterLimit) != null && ((F = e.chatInput) == null ? void 0 : F.characterLimit) > 0 && /* @__PURE__ */ G(
            "div",
            {
              className: "rcb-chat-input-char-counter",
              style: a >= ((Y = e.chatInput) == null ? void 0 : Y.characterLimit) ? f : I,
              children: [
                a,
                "/",
                (_ = e.chatInput) == null ? void 0 : _.characterLimit
              ]
            }
          )
        ] })
      ]
    }
  );
};
const qe = ({ buttons: A }) => {
  var o;
  const { settings: t } = x(), { styles: e } = W();
  return /* @__PURE__ */ G("div", { "data-testid": "chatbot-footer-container", style: e.footerStyle, className: "rcb-chat-footer-container", children: [
    /* @__PURE__ */ l("div", { className: "rcb-chat-footer", children: A == null ? void 0 : A.map(
      (n, s) => /* @__PURE__ */ l(ZA, { children: n }, s)
    ) }),
    /* @__PURE__ */ l("span", { children: (o = t.footer) == null ? void 0 : o.text })
  ] });
};
const _e = () => {
  var c, i, u, d, h, w, v;
  const { settings: A } = x(), { styles: t } = W(), { unreadCount: e } = AA(), { isChatWindowOpen: o, toggleChatWindow: n } = dA(), s = {
    backgroundImage: `linear-gradient(to right, ${(c = A.general) == null ? void 0 : c.secondaryColor},
			${(i = A.general) == null ? void 0 : i.primaryColor})`,
    ...t.chatButtonStyle
  }, a = {
    backgroundImage: `url(${(u = A.chatButton) == null ? void 0 : u.icon})`,
    fill: "#fff",
    width: 75,
    height: 75,
    ...t.chatIconStyle
  }, r = () => {
    var b;
    const m = (b = A.chatButton) == null ? void 0 : b.icon;
    return !m || typeof m == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-toggle-icon",
        style: a
      }
    ) : m && /* @__PURE__ */ l("span", { className: "rcb-toggle-icon", children: /* @__PURE__ */ l(m, { style: a }) });
  };
  return /* @__PURE__ */ l(sA, { children: !((d = A.general) != null && d.embedded) && /* @__PURE__ */ G(
    "div",
    {
      "aria-label": ((h = A.ariaLabel) == null ? void 0 : h.chatButton) ?? "open chat",
      role: "button",
      style: s,
      className: `rcb-toggle-button ${o ? "rcb-button-hide" : "rcb-button-show"}`,
      onClick: n,
      children: [
        r(),
        !((w = A.notification) != null && w.disabled) && ((v = A.notification) == null ? void 0 : v.showCount) && /* @__PURE__ */ l("span", { style: t.notificationBadgeStyle, className: "rcb-badge", children: e })
      ]
    }
  ) });
};
const $e = () => {
  var w, v, m, b;
  const A = xA(), { settings: t } = x(), { styles: e } = W(), { isChatWindowOpen: o, openChat: n } = dA(), [s, a] = H(!1), [r, c] = H(!1), [i, u] = H(0);
  K(() => {
    var D, E, I;
    const g = (D = t.tooltip) == null ? void 0 : D.mode;
    if (g === "ALWAYS")
      if (A) {
        let f;
        o ? f = (((E = e.chatWindowStyle) == null ? void 0 : E.width) ?? 375) - (((I = e.chatButtonStyle) == null ? void 0 : I.width) ?? 75) : f = 0, u(f), a(!0);
      } else
        a(!o);
    else
      g === "NEVER" ? a(!1) : g === "START" ? r ? a(!1) : (c(!0), a(!0)) : g === "CLOSE" && a(!o);
  }, [o]);
  const d = {
    transform: `translateX(-${i}px)`,
    right: (((w = e.chatButtonStyle) == null ? void 0 : w.width) ?? 75) + 40,
    bottom: 30,
    backgroundColor: (v = t.general) == null ? void 0 : v.secondaryColor,
    color: "#fff",
    ...e.tooltipStyle
  }, h = {
    borderColor: `transparent transparent transparent ${d.backgroundColor}`
  };
  return /* @__PURE__ */ l(sA, { children: !((m = t.general) != null && m.embedded) && /* @__PURE__ */ G(
    "div",
    {
      "data-testid": "chat-tooltip",
      style: d,
      className: `rcb-chat-tooltip ${s ? "rcb-tooltip-show" : "rcb-tooltip-hide"}`,
      onClick: () => n(!0),
      children: [
        /* @__PURE__ */ l("span", { children: (b = t.tooltip) == null ? void 0 : b.text }),
        /* @__PURE__ */ l("span", { className: "rcb-chat-tooltip-tail", style: h })
      ]
    }
  ) });
};
const Ao = ({
  id: A,
  content: t,
  timeout: e
}) => {
  var h, w;
  const { settings: o } = x(), { styles: n } = W(), [s] = H({
    toastPromptStyle: { ...n.toastPromptStyle },
    toastPromptHoveredStyle: { ...n.toastPromptHoveredStyle }
  }), { dismissToast: a } = CA(), [r, c] = H(!1), i = {
    color: (h = o.general) == null ? void 0 : h.primaryColor,
    borderColor: (w = o.general) == null ? void 0 : w.primaryColor,
    ...s.toastPromptHoveredStyle
  };
  return K(() => {
    if (e) {
      const v = setTimeout(() => {
        a(A);
      }, e);
      return () => clearTimeout(v);
    }
  }, [A, a, e]), typeof t == "string" ? /* @__PURE__ */ l(
    "div",
    {
      onMouseEnter: () => {
        c(!0);
      },
      onMouseLeave: () => {
        c(!1);
      },
      style: r ? i : s.toastPromptStyle,
      onMouseDown: async (v) => {
        var m;
        (m = o.toast) != null && m.dismissOnClick && (v.preventDefault(), await a(A));
      },
      className: "rcb-toast-prompt",
      children: t
    }
  ) : /* @__PURE__ */ l(sA, { children: t });
};
const to = () => {
  var o, n, s, a;
  const { styles: A } = W(), { toasts: t } = Ct(), e = {
    bottom: (((o = A.footerStyle) == null ? void 0 : o.height) ?? 50) + (((n = A.chatInputContainerStyle) == null ? void 0 : n.height) ?? 70) + 20,
    width: 300,
    minWidth: (((s = A.chatWindowStyle) == null ? void 0 : s.width) ?? 375) / 2,
    maxWidth: (((a = A.chatWindowStyle) == null ? void 0 : a.width) ?? 375) - 50,
    ...A.toastPromptContainerStyle
  };
  return /* @__PURE__ */ l("div", { className: "rcb-toast-prompt-container", style: e, children: t.map((r) => /* @__PURE__ */ l(
    Ao,
    {
      id: r.id,
      content: r.content,
      timeout: r.timeout
    },
    r.id
  )) });
};
const It = ({
  file: A,
  fileType: t,
  fileUrl: e
}) => {
  var a, r, c;
  const { settings: o } = x(), { styles: n } = W(), s = {
    backgroundColor: (a = o.general) == null ? void 0 : a.primaryColor,
    maxWidth: (r = o.userBubble) != null && r.showAvatar ? "65%" : "70%",
    ...n.mediaDisplayContainerStyle
  };
  return /* @__PURE__ */ l(sA, { children: e ? /* @__PURE__ */ G(sA, { children: [
    t === "image" && e && /* @__PURE__ */ l(
      "div",
      {
        style: s,
        className: "rcb-media-display-image-container rcb-media-entry",
        "data-testid": "media-display-image-container",
        children: /* @__PURE__ */ l("img", { src: e, alt: "Image Display", className: "rcb-media-display-image" })
      }
    ),
    t === "video" && e && /* @__PURE__ */ l(
      "div",
      {
        style: s,
        className: "rcb-media-display-video-container rcb-media-entry",
        "data-testid": "media-display-video-container",
        children: /* @__PURE__ */ G("video", { controls: !0, className: "rcb-media-display-video", children: [
          /* @__PURE__ */ l("source", { src: e, type: A.type }),
          "Your browser does not support the video tag."
        ] })
      }
    ),
    t === "audio" && e && /* @__PURE__ */ G(
      "audio",
      {
        "data-testid": "media-display-audio-container",
        style: {
          maxWidth: (c = o.userBubble) != null && c.showAvatar ? "65%" : "70%"
        },
        controls: !0,
        className: "rcb-media-display-audio rcb-media-entry",
        children: [
          /* @__PURE__ */ l("source", { src: e, type: A.type }),
          "Your browser does not support the audio tag."
        ]
      }
    )
  ] }) : /* @__PURE__ */ l(sA, {}) });
}, pt = async (A) => {
  if (!A)
    return { fileType: null, fileUrl: null };
  const t = A.type.split("/")[0];
  if (!["image", "video", "audio"].includes(t))
    return { fileType: null, fileUrl: null };
  try {
    const e = await new Promise((o, n) => {
      const s = new FileReader();
      s.onload = () => o(s.result), s.onerror = () => n(new Error("File reading failed")), s.readAsDataURL(A);
    });
    return { fileType: t, fileUrl: e };
  } catch {
    return { fileType: null, fileUrl: null };
  }
};
const eo = () => {
  var y, p, T, M, L, C;
  const { settings: A } = x(), { styles: t } = W(), { injectMessage: e, streamMessage: o, removeMessage: n, endStreamMessage: s } = yA(), { getCurrPath: a, getPrevPath: r, goToPath: c, blockAllowsAttachment: i } = SA(), { flowRef: u, inputRef: d } = q(), h = u.current, { showToast: w, dismissToast: v } = CA(), { callRcbEvent: m } = uA(), { openChat: b } = dA(), { setTextAreaValue: g } = EA(), { handleSubmitText: D } = IA(), E = {
    cursor: `url(${(y = A.general) == null ? void 0 : y.actionDisabledIcon}), auto`,
    ...t.fileAttachmentButtonStyle,
    // by default inherit the base style
    ...t.fileAttachmentButtonDisabledStyle
  }, I = {
    backgroundImage: `url(${(p = A.fileAttachment) == null ? void 0 : p.icon})`,
    fill: "#a6a6a6",
    ...t.fileAttachmentIconStyle
  }, f = {
    backgroundImage: `url(${(T = A.fileAttachment) == null ? void 0 : T.icon})`,
    fill: "#a6a6a6",
    ...t.fileAttachmentIconStyle,
    // by default inherit the base style
    ...t.fileAttachmentIconDisabledStyle
  }, P = async (S) => {
    var z, Z, F, Y;
    const N = S.target.files;
    if (!N || (z = A.event) != null && z.rcbUserUploadFile && (await m(k.USER_UPLOAD_FILE, { files: N })).defaultPrevented)
      return;
    const j = a();
    if (!j)
      return;
    const U = h[j];
    if (!U)
      return;
    const R = U.file;
    if (R != null) {
      const _ = [];
      for (let X = 0; X < N.length; X++) {
        if (_.push(N[X].name), !((Z = A.fileAttachment) != null && Z.showMediaDisplay))
          continue;
        const V = await pt(N[X]);
        !V.fileType || !V.fileUrl || await e(/* @__PURE__ */ l(
          It,
          {
            file: N[X],
            fileType: V.fileType,
            fileUrl: V.fileUrl
          }
        ), "user");
      }
      await D("📄 " + _.join(", "), (F = A.fileAttachment) == null ? void 0 : F.sendFileName), await R({
        userInput: (Y = d.current) == null ? void 0 : Y.value,
        prevPath: r(),
        currPath: a(),
        goToPath: c,
        setTextAreaValue: g,
        injectMessage: e,
        streamMessage: o,
        removeMessage: n,
        endStreamMessage: s,
        openChat: b,
        showToast: w,
        dismissToast: v,
        files: N
      });
    }
  }, B = () => {
    var N, j;
    const S = i ? (N = A.fileAttachment) == null ? void 0 : N.icon : (j = A.fileAttachment) == null ? void 0 : j.iconDisabled;
    return !S || typeof S == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: i ? "rcb-attach-icon-enabled" : "rcb-attach-icon-disabled",
        style: i ? I : f
      }
    ) : S && /* @__PURE__ */ l("span", { className: i ? "rcb-attach-icon-enabled" : "rcb-attach-icon-disabled", children: /* @__PURE__ */ l(
      S,
      {
        style: i ? I : f
      }
    ) });
  };
  return /* @__PURE__ */ G(
    "label",
    {
      "aria-label": ((M = A.ariaLabel) == null ? void 0 : M.fileAttachmentButton) ?? "upload file",
      role: "button",
      className: i ? "rcb-attach-button-enabled" : "rcb-attach-button-disabled",
      style: i ? t.fileAttachmentButtonStyle : E,
      children: [
        /* @__PURE__ */ l(
          "input",
          {
            className: "rcb-attach-input",
            type: "file",
            onChange: P,
            multiple: (L = A.fileAttachment) == null ? void 0 : L.multiple,
            accept: (C = A.fileAttachment) == null ? void 0 : C.accept,
            disabled: !i
          }
        ),
        B()
      ]
    }
  );
};
const oo = () => {
  var m, b, g, D, E, I;
  const { settings: A } = x(), { styles: t } = W(), { inputRef: e } = q(), { textAreaDisabled: o, setTextAreaValue: n } = EA(), s = tA(null), a = tA(null), [r, c] = H(!1), i = {
    cursor: `url(${(m = A.general) == null ? void 0 : m.actionDisabledIcon}), auto`,
    ...t.emojiButtonStyle,
    // by default inherit the base style
    ...t.emojiButtonDisabledStyle
  }, u = {
    backgroundImage: `url(${(b = A.emoji) == null ? void 0 : b.icon})`,
    fill: "#a6a6a6",
    ...t.emojiIconStyle
  }, d = {
    backgroundImage: `url(${(g = A.emoji) == null ? void 0 : g.icon})`,
    fill: "#a6a6a6",
    ...t.emojiIconStyle,
    // by default inherit the base style
    ...t.emojiIconDisabledStyle
  };
  K(() => {
    const f = (y) => {
      const p = y.composedPath();
      s.current && !p.includes(s.current) && a.current && !p.includes(a.current) && c(!1);
    }, P = () => {
      if (s.current && a.current) {
        const y = a.current.getBoundingClientRect(), p = s.current.offsetHeight, T = y.top - p - 8;
        s.current.style.left = `${y.left}px`, s.current.style.top = `${T}px`;
      }
    }, B = () => {
      P();
    };
    return document.addEventListener("mousedown", f), window.addEventListener("resize", B), () => {
      document.removeEventListener("mousedown", f), window.removeEventListener("resize", B);
    };
  }, []);
  const h = (f, P) => {
    f.preventDefault(), e.current && (n(e.current.value + P), setTimeout(() => {
      const B = e.current;
      if (B) {
        B.focus();
        const y = B.value.length;
        B.setSelectionRange(y, y);
      }
    }, 50)), c(!1);
  }, w = (f) => {
    f.preventDefault(), c(o ? !1 : !r);
  }, v = () => {
    var P, B;
    const f = o ? (P = A.emoji) == null ? void 0 : P.iconDisabled : (B = A.emoji) == null ? void 0 : B.icon;
    return !f || typeof f == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: `${o ? "rcb-emoji-icon-disabled" : "rcb-emoji-icon-enabled"}`,
        style: o ? d : u
      }
    ) : f && /* @__PURE__ */ l("span", { className: `${o ? "rcb-emoji-icon-disabled" : "rcb-emoji-icon-enabled"}`, children: /* @__PURE__ */ l(f, { style: o ? d : u }) });
  };
  return /* @__PURE__ */ G(sA, { children: [
    /* @__PURE__ */ l(
      "div",
      {
        "aria-label": ((D = A.ariaLabel) == null ? void 0 : D.emojiButton) ?? "emoji picker",
        role: "button",
        ref: a,
        className: `${o ? "rcb-emoji-button-disabled" : "rcb-emoji-button-enabled"}`,
        style: o ? i : t.emojiButtonStyle,
        onMouseDown: w,
        children: v()
      }
    ),
    r && /* @__PURE__ */ l("div", { className: "rcb-emoji-button-popup", ref: s, children: (I = (E = A.emoji) == null ? void 0 : E.list) == null ? void 0 : I.map((f, P) => /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-emoji",
        onMouseDown: (B) => h(B, f),
        children: f
      },
      P
    )) })
  ] });
}, Qt = () => {
  const { settings: A } = x(), { audioToggledOn: t, setAudioToggledOn: e } = AA(), { callRcbEvent: o } = uA(), n = O(async () => {
    var s;
    (s = A.event) != null && s.rcbToggleAudio && (await o(
      k.TOGGLE_AUDIO,
      { currState: t, newState: !t }
    )).defaultPrevented || e((a) => !a);
  }, [t]);
  return {
    audioToggledOn: t,
    toggleAudio: n
  };
};
const no = () => {
  var r, c, i;
  const { settings: A } = x(), { styles: t } = W(), { audioToggledOn: e, toggleAudio: o } = Qt(), n = {
    backgroundImage: `url(${(r = A.audio) == null ? void 0 : r.icon})`,
    fill: "#fcec3d",
    ...t.audioIconStyle
  }, s = {
    backgroundImage: `url(${(c = A.audio) == null ? void 0 : c.iconDisabled})`,
    fill: "#e8eaed",
    ...t.audioIconStyle,
    // by default inherit the base style
    ...t.audioIconDisabledStyle
  }, a = () => {
    var d, h;
    const u = e ? (d = A.audio) == null ? void 0 : d.icon : (h = A.audio) == null ? void 0 : h.iconDisabled;
    return !u || typeof u == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-audio-icon",
        "data-testid": "rcb-audio-icon",
        style: e ? n : s
      }
    ) : u && /* @__PURE__ */ l("span", { className: "rcb-audio-icon", "data-testid": "rcb-audio-icon", children: /* @__PURE__ */ l(
      u,
      {
        style: e ? n : s,
        "data-testid": "rcb-audio-icon-svg"
      }
    ) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((i = A.ariaLabel) == null ? void 0 : i.audioButton) ?? "toggle audio",
      role: "button",
      onMouseDown: async (u) => {
        u.preventDefault(), await o();
      },
      style: e ? t.audioButtonStyle : { ...t.audioButtonStyle, ...t.audioButtonDisabledStyle },
      children: a()
    }
  );
};
const so = () => {
  var s, a;
  const { settings: A } = x(), { styles: t } = W(), { openChat: e } = dA(), o = {
    backgroundImage: `url(${(s = A.header) == null ? void 0 : s.closeChatIcon})`,
    fill: "#e8eaed",
    stroke: "#e8eaed",
    ...t.closeChatIconStyle
  }, n = () => {
    var c;
    const r = (c = A.header) == null ? void 0 : c.closeChatIcon;
    return !r || typeof r == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-close-chat-icon",
        "data-testid": "rcb-close-chat-icon",
        style: o
      }
    ) : r && /* @__PURE__ */ l("span", { className: "rcb-close-chat-icon", "data-testid": "rcb-close-chat-icon", children: /* @__PURE__ */ l(r, { style: o }) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((a = A.ariaLabel) == null ? void 0 : a.closeChatButton) ?? "close chat",
      role: "button",
      onMouseDown: (r) => {
        r.stopPropagation(), e(!1);
      },
      style: t.closeChatButtonStyle,
      children: n()
    }
  );
}, ro = () => {
  const { notificationsToggledOn: A, toggleNotifications: t, playNotificationSound: e } = XA();
  return {
    notificationsToggledOn: A,
    toggleNotifications: t,
    playNotificationSound: e
  };
};
const ao = () => {
  var r, c, i;
  const { settings: A } = x(), { styles: t } = W(), { notificationsToggledOn: e, toggleNotifications: o } = ro(), n = {
    backgroundImage: `url(${(r = A.notification) == null ? void 0 : r.icon})`,
    fill: "#fcec3d",
    ...t.notificationIconStyle
  }, s = {
    backgroundImage: `url(${(c = A.notification) == null ? void 0 : c.iconDisabled})`,
    fill: "#e8eaed",
    ...t.notificationIconStyle,
    // by default inherit the base style
    ...t.notificationIconDisabledStyle
  }, a = () => {
    var d, h;
    const u = e ? (d = A.notification) == null ? void 0 : d.icon : (h = A.notification) == null ? void 0 : h.iconDisabled;
    return !u || typeof u == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-notification-icon",
        "data-testid": "rcb-notification-icon",
        style: e ? n : s
      }
    ) : u && /* @__PURE__ */ l("span", { className: "rcb-notification-icon", "data-testid": "rcb-notification-icon", children: /* @__PURE__ */ l(
      u,
      {
        style: e ? n : s,
        "data-testid": "rcb-notification-icon-svg"
      }
    ) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((i = A.ariaLabel) == null ? void 0 : i.notificationButton) ?? "toggle notifications",
      role: "button",
      onMouseDown: async (u) => {
        u.preventDefault(), await o();
      },
      style: e ? t.notificationButtonStyle : { ...t.notificationButtonStyle, ...t.notificationButtonDisabledStyle },
      children: a()
    }
  );
};
const co = () => {
  var D, E, I;
  const { settings: A } = x(), { styles: t } = W(), { injectMessage: e } = yA(), { inputRef: o } = q(), { voiceToggledOn: n, toggleVoice: s } = LA(), { setInputLength: a, setTextAreaValue: r, textAreaDisabled: c } = EA(), { handleSubmitText: i } = IA(), u = tA([]), [d, h] = H(!1);
  K(() => {
    var f;
    (f = A.voice) != null && f.sendAsAudio ? (b(), u.current = []) : i();
  }, [d]), K(() => {
    n ? We(
      A,
      s,
      m,
      r,
      a,
      u,
      o
    ) : VA();
  }, [n]);
  const w = {
    backgroundImage: `url(${(D = A.voice) == null ? void 0 : D.icon})`,
    fill: "#9aa0a6",
    ...t.voiceIconStyle
  }, v = {
    backgroundImage: `url(${(E = A.voice) == null ? void 0 : E.iconDisabled})`,
    fill: "#9aa0a6",
    ...t.voiceIconStyle,
    // by default inherit the base style
    ...t.voiceIconDisabledStyle
  }, m = () => {
    h((f) => !f);
  }, b = async () => {
    const f = new Blob(u.current, { type: "audio/wav" }), P = new File([f], "voice-input.wav", { type: "audio/wav" }), B = await pt(P);
    !B.fileType || !B.fileUrl || await e(/* @__PURE__ */ l(
      It,
      {
        file: P,
        fileType: B.fileType,
        fileUrl: B.fileUrl
      }
    ), "user");
  }, g = () => {
    var P, B;
    const f = n ? (P = A.voice) == null ? void 0 : P.icon : (B = A.voice) == null ? void 0 : B.iconDisabled;
    return !f || typeof f == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: `rcb-voice-icon${n && !c ? "-on" : ""}`,
        style: n && !c ? w : v
      }
    ) : f && /* @__PURE__ */ l("span", { className: `rcb-voice-icon ${n && !c ? "on" : ""}`, children: /* @__PURE__ */ l(f, { style: n && !c ? w : v }) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((I = A.ariaLabel) == null ? void 0 : I.voiceButton) ?? "toggle voice",
      role: "button",
      onMouseDown: async (f) => {
        f.preventDefault(), !c && await s();
      },
      style: n && !c ? t.voiceButtonStyle : { ...t.voiceButtonStyle, ...t.voiceButtonDisabledStyle },
      className: n && !c ? "rcb-voice-button-enabled" : "rcb-voice-button-disabled",
      children: g()
    }
  );
};
const io = () => {
  var m, b, g, D, E, I, f, P;
  const { settings: A } = x(), { styles: t } = W(), { textAreaDisabled: e } = AA(), [o, n] = H(!1), { handleSubmitText: s } = IA(), a = {
    backgroundColor: (m = A.general) == null ? void 0 : m.primaryColor,
    ...t.sendButtonStyle
  }, r = {
    cursor: `url(${(b = A.general) == null ? void 0 : b.actionDisabledIcon}), auto`,
    backgroundColor: (g = A.general) == null ? void 0 : g.primaryColor,
    ...t.sendButtonStyle,
    // by default inherit the base style
    ...t.sendButtonDisabledStyle
  }, c = {
    backgroundColor: (D = A.general) == null ? void 0 : D.secondaryColor,
    ...t.sendButtonStyle,
    // by default inherit the base style
    ...t.sendButtonHoveredStyle
  }, i = {
    backgroundImage: `url(${(E = A.chatInput) == null ? void 0 : E.sendButtonIcon})`,
    fill: "#fff",
    ...t.sendIconStyle
  }, u = {
    backgroundImage: `url(${(I = A.chatInput) == null ? void 0 : I.sendButtonIcon})`,
    fill: "#fff",
    ...t.sendIconStyle,
    // by default inherit the base style
    ...t.sendIconDisabledStyle
  }, d = {
    backgroundImage: `url(${(f = A.chatInput) == null ? void 0 : f.sendButtonIcon})`,
    fill: "#fff",
    ...t.sendIconStyle,
    // by default inherit the base style
    ...t.sendIconHoveredStyle
  }, h = () => {
    n(!0);
  }, w = () => {
    n(!1);
  }, v = () => {
    var y;
    const B = (y = A.chatInput) == null ? void 0 : y.sendButtonIcon;
    return !B || typeof B == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-send-icon",
        "data-testid": "rcb-send-icon",
        style: e ? u : o ? d : i
      }
    ) : B && /* @__PURE__ */ l("span", { className: "rcb-send-icon", "data-testid": "rcb-send-icon", children: /* @__PURE__ */ l(
      B,
      {
        style: e ? u : o ? d : i
      }
    ) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((P = A.ariaLabel) == null ? void 0 : P.sendButton) ?? "send message",
      role: "button",
      onMouseEnter: h,
      onMouseLeave: w,
      onMouseDown: async (B) => {
        B == null || B.preventDefault(), !e && await s();
      },
      style: e ? r : o ? c : a,
      className: "rcb-send-button",
      children: v()
    }
  );
}, lo = (A) => Object.values(J).includes(A), uo = (A, t) => {
  var c, i, u, d, h, w, v, m, b;
  const e = {
    [J.AUDIO_BUTTON]: (c = A.audio) == null ? void 0 : c.disabled,
    [J.CLOSE_CHAT_BUTTON]: (i = A.general) == null ? void 0 : i.embedded,
    [J.EMOJI_PICKER_BUTTON]: (u = A.emoji) == null ? void 0 : u.disabled,
    [J.FILE_ATTACHMENT_BUTTON]: (d = A.fileAttachment) == null ? void 0 : d.disabled,
    [J.NOTIFICATION_BUTTON]: (h = A.notification) == null ? void 0 : h.disabled,
    [J.SEND_MESSAGE_BUTTON]: !1,
    [J.VOICE_MESSAGE_BUTTON]: (w = A.voice) == null ? void 0 : w.disabled
  }, o = {}, n = (g) => g ? g.map((D) => {
    if (typeof D == "string") {
      if (lo(D) && !e[D]) {
        if (o[D])
          return o[D];
        const E = t[D];
        if (typeof E == "function") {
          const I = E();
          return o[D] = I, I;
        }
        return null;
      }
      return null;
    }
    return D;
  }).filter((D) => D !== null) : [], s = n((v = A.header) == null ? void 0 : v.buttons), a = n((m = A.chatInput) == null ? void 0 : m.buttons), r = n((b = A.footer) == null ? void 0 : b.buttons);
  return { header: s, chatInput: a, footer: r };
}, wo = () => /* @__PURE__ */ l(no, {}), fo = () => /* @__PURE__ */ l(ao, {}), go = () => /* @__PURE__ */ l(so, {}), Bo = () => /* @__PURE__ */ l(co, {}), vo = () => /* @__PURE__ */ l(io, {}), Do = () => /* @__PURE__ */ l(eo, {}), bo = () => /* @__PURE__ */ l(oo, {}), ho = () => {
  const { settings: A } = x(), t = fA(() => ({
    [J.CLOSE_CHAT_BUTTON]: () => go(),
    [J.AUDIO_BUTTON]: () => wo(),
    [J.NOTIFICATION_BUTTON]: () => fo(),
    [J.EMOJI_PICKER_BUTTON]: () => bo(),
    [J.FILE_ATTACHMENT_BUTTON]: () => Do(),
    [J.SEND_MESSAGE_BUTTON]: () => vo(),
    [J.VOICE_MESSAGE_BUTTON]: () => Bo()
  }), []), { header: e, chatInput: o, footer: n } = fA(() => uo(A, t), [A, t]), s = fA(() => e, [e]), a = fA(() => o, [o]), r = fA(() => n, [n]);
  return { headerButtons: s, chatInputButtons: a, footerButtons: r };
}, KA = () => {
  const { settings: A } = x(), { styles: t } = W(), { setMessages: e } = WA(), {
    isLoadingChatHistory: o,
    setIsLoadingChatHistory: n
  } = AA(), { chatBodyRef: s } = q(), { callRcbEvent: a } = uA(), { chatScrollHeight: r } = dA(), c = O(async () => {
    var u;
    const i = YA();
    i && ((u = A.event) != null && u.rcbLoadChatHistory && (await a(k.LOAD_CHAT_HISTORY, {})).defaultPrevented || (n(!0), Fe(
      A,
      t,
      i,
      e,
      s,
      r,
      n
    )));
  }, [A, t, e]);
  return { isLoadingChatHistory: o, setIsLoadingChatHistory: n, showChatHistory: c };
};
const mo = () => {
  var c, i, u;
  const { settings: A } = x(), { styles: t } = W(), { showChatHistory: e } = KA(), [o, n] = H(!1), s = {
    color: (c = A.general) == null ? void 0 : c.primaryColor,
    borderColor: (i = A.general) == null ? void 0 : i.primaryColor,
    ...t.chatHistoryButtonStyle,
    // by default inherit the base style
    ...t.chatHistoryButtonHoveredStyle
  };
  return /* @__PURE__ */ l("div", { className: "rcb-view-history-container", children: /* @__PURE__ */ l(
    "div",
    {
      onMouseEnter: () => {
        n(!0);
      },
      onMouseLeave: () => {
        n(!1);
      },
      style: o ? s : t.chatHistoryButtonStyle,
      onMouseDown: (d) => {
        d.preventDefault(), e();
      },
      className: "rcb-view-history-button",
      role: "button",
      tabIndex: 0,
      children: /* @__PURE__ */ l("p", { children: (u = A.chatHistory) == null ? void 0 : u.viewChatHistoryButtonText })
    }
  ) });
}, St = () => {
  var a;
  const { settings: A } = x(), { hasInteractedPage: t, setHasInteractedPage: e, hasFlowStarted: o, setHasFlowStarted: n } = AA(), s = O(() => {
    var c;
    if (e(!0), !o && ((c = A.general) == null ? void 0 : c.flowStartTrigger) === "ON_PAGE_INTERACT" && n(!0), !window.SpeechSynthesisUtterance) {
      console.info("Speech Synthesis API is not supported in this environment.");
      return;
    }
    const r = new window.SpeechSynthesisUtterance();
    r.text = "", r.onend = () => {
      window.removeEventListener("click", s), window.removeEventListener("keydown", s), window.removeEventListener("touchstart", s);
    }, window.speechSynthesis.speak(r);
  }, [o, (a = A.general) == null ? void 0 : a.flowStartTrigger]);
  return {
    hasInteractedPage: t,
    handleFirstInteraction: s
  };
}, Po = () => {
  var pA, mA, _A, $A;
  const A = xA(), { settings: t } = x(), {
    endStreamMessage: e,
    injectMessage: o,
    removeMessage: n,
    streamMessage: s,
    replaceMessages: a
  } = yA(), { getCurrPath: r, getPrevPath: c, goToPath: i, paths: u } = SA(), { showToast: d, dismissToast: h } = CA(), {
    isBotTyping: w,
    isChatWindowOpen: v,
    isScrolling: m,
    hasFlowStarted: b,
    setIsChatWindowOpen: g,
    setTextAreaDisabled: D,
    setAudioToggledOn: E,
    setVoiceToggledOn: I,
    setIsBotTyping: f,
    setTextAreaSensitiveMode: P,
    setBlockAllowsAttachment: B,
    setTimeoutId: y
  } = AA(), { chatBodyRef: p, flowRef: T, streamMessageMap: M, paramsInputRef: L, keepVoiceOnRef: C } = q(), S = T.current, { viewportHeight: N, setViewportHeight: j, setViewportWidth: U, openChat: R } = dA(), { setUpNotifications: z } = XA(), { handleFirstInteraction: Z } = St(), { showChatHistory: F } = KA(), { updateTextAreaFocus: Y, setTextAreaValue: _ } = EA(), { syncVoice: X } = LA(), V = tA(0);
  K(() => (window.addEventListener("click", Z), window.addEventListener("keydown", Z), window.addEventListener("touchstart", Z), () => {
    window.removeEventListener("click", Z), window.removeEventListener("keydown", Z), window.removeEventListener("touchstart", Z);
  }), []), K(() => {
    var $, eA, rA;
    z(), D(($ = t.chatInput) == null ? void 0 : $.disabled), g((eA = t.chatWindow) == null ? void 0 : eA.defaultOpen), E((rA = t.audio) == null ? void 0 : rA.defaultToggledOn), setTimeout(() => {
      var aA;
      I((aA = t.voice) == null ? void 0 : aA.defaultToggledOn);
    }, 1);
  }, []), K(() => {
    !m && (p != null && p.current) && (p.current.scrollTop = p.current.scrollHeight);
  }, [w]), K(() => {
    var $, eA;
    if (($ = t.chatHistory) != null && $.disabled)
      Le();
    else if (xe(t), YA().length > 0) {
      const aA = vA(/* @__PURE__ */ l(mo, {}), "system");
      a([aA]), (eA = t.chatHistory) != null && eA.autoLoad && F();
    }
  }, [(pA = t.chatHistory) == null ? void 0 : pA.storageKey, (mA = t.chatHistory) == null ? void 0 : mA.maxEntries, (_A = t.chatHistory) == null ? void 0 : _A.disabled]), K(() => {
    var $;
    A || ($ = t.general) != null && $.embedded || !navigator.virtualKeyboard || (navigator.virtualKeyboard.overlaysContent = !0, navigator.virtualKeyboard.addEventListener("geometrychange", (eA) => {
      if (!eA.target)
        return;
      const { x: rA, y: aA, width: PA, height: wA } = eA.target.boundingRect;
      rA == 0 && aA == 0 && PA == 0 && wA == 0 ? (setTimeout(() => {
        var nA;
        j((nA = window.visualViewport) == null ? void 0 : nA.height);
      }, 101), setTimeout(() => {
        var nA, BA;
        N != ((nA = window.visualViewport) == null ? void 0 : nA.height) && j((BA = window.visualViewport) == null ? void 0 : BA.height);
      }, 1001)) : setTimeout(() => {
        var nA;
        j(((nA = window.visualViewport) == null ? void 0 : nA.height) - wA);
      }, 101);
    }));
  }, [A]), K(() => {
    var PA, wA, nA, BA;
    if (A)
      return;
    v && (j((PA = window.visualViewport) == null ? void 0 : PA.height), U((wA = window.visualViewport) == null ? void 0 : wA.width));
    const $ = () => window.scrollTo({ top: 0, left: 0, behavior: "auto" }), eA = () => V.current = window.scrollY, rA = () => {
      var QA, At;
      j((QA = window.visualViewport) == null ? void 0 : QA.height), U((At = window.visualViewport) == null ? void 0 : At.width);
    }, aA = () => {
      var QA;
      window.removeEventListener("scroll", $), window.removeEventListener("scroll", eA), (QA = window.visualViewport) == null || QA.removeEventListener("resize", rA);
    };
    return v ? (aA(), document.body.style.position = "fixed", window.addEventListener("scroll", $), (nA = window.visualViewport) == null || nA.addEventListener("resize", rA)) : (document.body.style.position = "static", aA(), window.scrollTo({ top: V.current, left: 0, behavior: "auto" }), window.addEventListener("scroll", eA), (BA = window.visualViewport) == null || BA.removeEventListener("resize", rA)), aA;
  }, [v, A]), K(() => {
    const $ = r();
    if (!$)
      return;
    const eA = S[$];
    if (!eA) {
      f(!1);
      return;
    }
    const rA = {
      prevPath: c(),
      currPath: r(),
      goToPath: i,
      setTextAreaValue: _,
      userInput: L.current,
      endStreamMessage: e,
      injectMessage: o,
      removeMessage: n,
      streamMessage: s,
      openChat: R,
      showToast: d,
      dismissToast: h
    };
    (async (PA, wA, nA) => {
      var BA;
      await Qe(
        S,
        PA,
        nA,
        D,
        P,
        i,
        y
      ), f(!1), "chatDisabled" in wA || D((BA = t.chatInput) == null ? void 0 : BA.disabled), B(typeof wA.file == "function"), Y(PA), X(C.current && !wA.chatDisabled), M.current.clear();
    })($, eA, rA);
  }, [u]), K(() => {
    var $;
    (b || (($ = t.general) == null ? void 0 : $.flowStartTrigger) === "ON_LOAD") && i("start");
  }, [b, ($A = t.general) == null ? void 0 : $A.flowStartTrigger]);
}, qA = {
  DEFAULT_URL: "https://cdn.jsdelivr.net/gh/tjtanjin/react-chatbotify-themes/themes",
  DEFAULT_EXPIRATION: "2592000",
  CACHE_KEY_PREFIX: "RCB_THEME_CACHE_DATA"
}, Eo = qA.DEFAULT_URL, yo = qA.DEFAULT_EXPIRATION, Tt = qA.CACHE_KEY_PREFIX, Co = (A, t, e) => {
  const o = localStorage.getItem(`${Tt}_${A}_${t}`);
  if (!o)
    return null;
  try {
    const n = JSON.parse(o), s = (/* @__PURE__ */ new Date()).getTime(), a = Math.floor(s / 1e3);
    return n.cacheDate + e >= a ? n : null;
  } catch (n) {
    return console.error(`Unable to fetch cache for ${A}`, n), null;
  }
}, Io = (A, t, e, o, n) => {
  const s = (/* @__PURE__ */ new Date()).getTime(), a = Math.floor(s / 1e3), r = {
    settings: e,
    inlineStyles: o,
    cssStylesText: n,
    cacheDate: a
  };
  localStorage.setItem(`${Tt}_${A}_${t}`, JSON.stringify(r));
}, po = async (A, t) => {
  const e = `${t}/${A}/meta.json`;
  try {
    const o = await fetch(e);
    return o.ok ? (await o.json()).version : (console.error(`Failed to fetch meta.json from ${e}`), null);
  } catch (o) {
    return console.error(`Failed to fetch meta.json from ${e}`, o), null;
  }
}, rt = async (A, t) => {
  const { id: e, version: o, baseUrl: n = Eo, cacheDuration: s = yo } = t, a = o || await po(e, n);
  if (!a)
    return console.error(`Unable to find version for theme: ${e}`), { settings: {}, inlineStyles: {}, cssStylesText: "" };
  const r = Co(e, a, s);
  if (r) {
    const D = at(A, r.cssStylesText);
    return { settings: r.settings, inlineStyles: r.inlineStyles, cssStylesText: D };
  }
  const c = `${n}/${e}/${a}/styles.css`, i = `${n}/${e}/${a}/settings.json`, u = `${n}/${e}/${a}/styles.json`;
  let d = "";
  const h = await fetch(c);
  h.ok ? d = await h.text() : console.info(`Could not fetch styles.css from ${c}`);
  const w = await fetch(i);
  let v = {};
  w.ok ? v = await w.json() : console.info(`Could not fetch settings.json from ${i}`);
  const m = await fetch(u);
  let b = {};
  m.ok ? b = await m.json() : console.info(`Could not fetch styles.json from ${u}`), Io(e, a, v, b, d);
  const g = at(A, d);
  return { settings: v, inlineStyles: b, cssStylesText: g };
}, at = (A, t) => t.split(new RegExp("(?<=})")).map((o) => {
  const n = o.trim();
  return n.startsWith("@import") || n.startsWith("@keyframes") || n.startsWith("@media") ? n : n ? `#${A} ${n}` : "";
}).join(`
`), Qo = () => RA(it), So = () => RA(ut), To = async (A, t, e, o) => {
  var r, c;
  let n = Qo(), s = So(), a = "";
  if (o != null)
    if (Array.isArray(o))
      for (const i of o) {
        const u = await rt(A, i);
        n = lA(u.settings, n), s = lA(u.inlineStyles, s), a += u.cssStylesText;
      }
    else {
      const i = await rt(A, o);
      n = lA(i.settings, n), s = lA(i.inlineStyles, s), a += i.cssStylesText;
    }
  return t != null && (n = lA(t, n)), e != null && (s = lA(e, s)), ((r = n.chatInput) == null ? void 0 : r.botDelay) != null && ((c = n.chatInput) == null ? void 0 : c.botDelay) < 500 && (n.chatInput.botDelay = 500), { settings: n, inlineStyles: s, cssStylesText: a };
}, lA = (A, t) => {
  const e = [{ source: A, target: t }];
  for (; e.length; ) {
    const o = e.pop();
    if (o == null)
      continue;
    const { source: n, target: s } = o;
    for (const a of Object.keys(n)) {
      const r = a;
      if (ct(n[r]))
        s[r] = n[r];
      else if (typeof n[r] == "object" && n[r] !== null && !Array.isArray(n[r]))
        (typeof s[r] != "object" || s[r] === null) && (s[r] = {}), e.push({ source: n[r], target: s[r] });
      else
        try {
          s[r] = n[r];
        } catch {
        }
    }
  }
  return t;
}, RA = (A) => {
  if (A === null || typeof A != "object")
    return A;
  const t = Array.isArray(A) ? [] : {}, e = [{ source: A, target: t }], o = /* @__PURE__ */ new WeakMap();
  for (o.set(A, t); e.length; ) {
    const n = e.pop();
    if (n == null)
      continue;
    const { source: s, target: a } = n;
    for (const r in s)
      if (Object.prototype.hasOwnProperty.call(s, r)) {
        const c = s[r];
        if (c && typeof c == "object")
          if (o.has(c))
            a[r] = o.get(c);
          else {
            const i = Array.isArray(c) ? [] : {};
            o.set(c, i), a[r] = i, e.push({ source: c, target: i });
          }
        else
          a[r] = c;
      }
  }
  return t;
}, Mt = () => {
  const { settings: A, setSettings: t } = x(), e = O((n) => {
    !n || Object.keys(n).length === 0 || t(RA(lA(n, A)));
  }, [A]), o = O((n) => {
    t(n);
  }, []);
  return {
    settings: A,
    replaceSettings: o,
    updateSettings: e
  };
}, Ht = () => {
  const { styles: A, setStyles: t } = W(), e = O((n) => {
    !n || Object.keys(n).length === 0 || t(RA(lA(n, A)));
  }, [A]), o = O((n) => {
    t(n);
  }, []);
  return {
    styles: A,
    replaceStyles: o,
    updateStyles: e
  };
}, Mo = (A) => {
  const { updateSettings: t } = Mt(), { updateStyles: e } = Ht(), o = A == null ? void 0 : A.map((n) => n());
  K(() => {
    let n = {}, s = {};
    o == null || o.forEach((a) => {
      a != null && a.settings && Object.keys(a == null ? void 0 : a.settings).length !== 0 && (n = lA(a.settings, n)), a != null && a.styles && Object.keys(a == null ? void 0 : a.styles).length !== 0 && (s = lA(a.styles, s));
    }), t(n), e(s);
  }, [A]);
};
const Ho = ({
  plugins: A
}) => {
  var g, D, E, I;
  const t = xA(), { settings: e } = x(), { styles: o } = W(), { hasFlowStarted: n, setHasFlowStarted: s } = AA(), { inputRef: a } = q(), {
    setChatScrollHeight: r,
    viewportHeight: c,
    viewportWidth: i,
    isChatWindowOpen: u
  } = dA(), { headerButtons: d, chatInputButtons: h, footerButtons: w } = ho();
  Po(), Mo(A);
  const v = fA(() => {
    var P;
    const f = "rcb-chatbot-global ";
    return (P = e.general) != null && P.embedded ? f + "rcb-window-embedded" : u ? f + "rcb-window-open" : f + "rcb-window-close";
  }, [e, u]), m = () => {
    var f;
    return !t && !((f = e.general) != null && f.embedded) ? {
      ...o.chatWindowStyle,
      borderRadius: "0px",
      left: "0px",
      right: "auto",
      top: "0px",
      bottom: "auto",
      width: `${i}px`,
      height: `${c}px`
    } : o.chatWindowStyle;
  };
  return /* @__PURE__ */ l(sA, { children: (() => {
    var f, P;
    return t && ((f = e.device) == null ? void 0 : f.desktopEnabled) || !t && ((P = e.device) == null ? void 0 : P.mobileEnabled);
  })() && /* @__PURE__ */ G(
    "div",
    {
      onMouseDown: (f) => {
        var P, B;
        !n && ((P = e.general) == null ? void 0 : P.flowStartTrigger) === "ON_CHATBOT_INTERACT" && s(!0), t ? (B = a.current) == null || B.blur() : f == null || f.preventDefault();
      },
      className: v,
      children: [
        /* @__PURE__ */ l($e, {}),
        /* @__PURE__ */ l(_e, {}),
        u && !t && !((g = e.general) != null && g.embedded) && /* @__PURE__ */ G(sA, { children: [
          /* @__PURE__ */ l("style", { children: `
									html {
										overflow: hidden !important;
										touch-action: none !important;
										scroll-behavior: auto !important;
									}
								` }),
          /* @__PURE__ */ l(
            "div",
            {
              style: {
                position: "fixed",
                top: 0,
                left: 0,
                width: "100%",
                height: "100%",
                backgroundColor: "#fff",
                zIndex: 9999
              }
            }
          )
        ] }),
        /* @__PURE__ */ G("div", { style: m(), className: "rcb-chat-window", children: [
          ((D = e.general) == null ? void 0 : D.showHeader) && /* @__PURE__ */ l(ie, { buttons: d }),
          /* @__PURE__ */ l(ve, { setChatScrollHeight: r }),
          /* @__PURE__ */ l(to, {}),
          ((E = e.general) == null ? void 0 : E.showInputRow) && /* @__PURE__ */ l(Ke, { buttons: h }),
          ((I = e.general) == null ? void 0 : I.showFooter) && /* @__PURE__ */ l(qe, { buttons: w })
        ] })
      ]
    }
  ) });
}, Ot = bA(void 0), Nt = () => hA(Ot), Oo = ({
  children: A
}) => {
  var u;
  const t = tA(""), e = tA({}), [o, n] = H({}), [s, a] = H({}), [r, c] = H(!1);
  K(() => {
    c(!0);
  }, []);
  const i = async (d, h, w, v, m, b) => {
    t.current = d, e.current = h;
    const g = await To(d, w, v, m);
    b.current && (b.current.textContent = g.cssStylesText), n(g.settings), a(g.inlineStyles);
  };
  return r ? /* @__PURE__ */ l("div", { style: { fontFamily: (u = o.general) == null ? void 0 : u.fontFamily }, children: /* @__PURE__ */ l(Ot.Provider, { value: { loadConfig: i }, children: /* @__PURE__ */ l(ae, { settings: o, setSettings: n, children: /* @__PURE__ */ l(ce, { styles: s, setStyles: a, children: /* @__PURE__ */ l(Ve, { children: /* @__PURE__ */ l(le, { botIdRef: t, flowRef: e, children: /* @__PURE__ */ l(fe, { children: /* @__PURE__ */ l(ue, { settings: o, children: /* @__PURE__ */ l(Be, { children: A }) }) }) }) }) }) }) }) }) : null;
}, No = ({
  id: A,
  flow: t,
  settings: e,
  styles: o,
  themes: n,
  plugins: s,
  setConfigLoaded: a,
  styleRootRef: r
}) => {
  const c = fA(() => A, []), i = Nt(), { flowRef: u } = q();
  u && u.current !== t && (u.current = t);
  const d = async () => {
    var h, w;
    (h = e.event) != null && h.rcbPreLoadChatBot && (await HA(
      k.PRE_LOAD_CHATBOT,
      { botId: c, currPath: null, prevPath: null },
      {
        flow: t,
        settings: e,
        styles: o,
        themes: n,
        plugins: s
      }
    )).defaultPrevented || (i != null && i.loadConfig && (await i.loadConfig(c, t, e, o, n, r), a(!0)), (w = e.event) != null && w.rcbPostLoadChatBot && HA(
      k.POST_LOAD_CHATBOT,
      { botId: c, currPath: null, prevPath: null },
      {
        flow: t,
        settings: e,
        styles: o,
        themes: n,
        plugins: s
      }
    ));
  };
  return K(() => {
    d();
  }, [n]), null;
}, jA = ["Quickstart", "API Docs", "Examples", "Github", "Discord"], ko = {
  start: {
    message: "Hello, I am Tan Jin 👋! Welcome to React ChatBotify, I'm excited that you are using our chatbot 😊!",
    transition: { duration: 1e3 },
    chatDisabled: !0,
    path: "show_options"
  },
  show_options: {
    message: "It looks like you have not set up a conversation flow yet. No worries! Here are a few helpful things you can check out to get started:",
    options: jA,
    path: "process_options"
  },
  prompt_again: {
    message: "Do you need any other help?",
    options: jA,
    path: "process_options"
  },
  unknown_input: {
    message: "Sorry, I do not understand your message 😢! If you require further assistance, you may click on the Github option and open an issue there or visit our discord.",
    options: jA,
    path: "process_options"
  },
  process_options: {
    transition: { duration: 0 },
    path: (A) => {
      let t = "";
      switch (A.userInput) {
        case "Quickstart":
          t = "https://react-chatbotify.com/docs/introduction/quickstart/";
          break;
        case "API Docs":
          t = "https://react-chatbotify.com/docs/api/bot_options";
          break;
        case "Examples":
          t = "https://react-chatbotify.com/docs/examples/basic_form";
          break;
        case "Github":
          t = "https://github.com/tjtanjin/react-chatbotify/";
          break;
        case "Discord":
          t = "https://discord.gg/6R4DK4G5Zh";
          break;
        default:
          return "unknown_input";
      }
      return A.injectMessage("Sit tight! I'll send you right there!"), setTimeout(() => {
        window.open(t);
      }, 1e3), "repeat";
    }
  },
  repeat: {
    transition: { duration: 3e3 },
    path: "prompt_again"
  }
}, jo = ({
  id: A,
  flow: t,
  settings: e,
  styles: o,
  themes: n,
  plugins: s
}) => {
  const a = fA(() => A || "rcb-" + OA(), []), r = !t || Object.keys(t).length === 0 ? ko : t, c = e || {}, i = o || {}, u = s || [], [d, h] = H(!1), w = Nt(), v = tA(null), [m, b] = H(n || []);
  K(() => {
    n && b(n);
  }, [n]);
  const g = () => /* @__PURE__ */ G(sA, { children: [
    /* @__PURE__ */ l(
      No,
      {
        styleRootRef: v,
        id: a,
        flow: r,
        settings: c,
        styles: i,
        themes: m,
        plugins: u,
        setConfigLoaded: h
      }
    ),
    d && /* @__PURE__ */ l(Ho, { plugins: s })
  ] });
  return w == null ? /* @__PURE__ */ G(Oo, { children: [
    /* @__PURE__ */ l("style", { ref: v }),
    /* @__PURE__ */ l("div", { id: a, children: g() })
  ] }) : /* @__PURE__ */ G(sA, { children: [
    /* @__PURE__ */ l("style", { ref: v }),
    /* @__PURE__ */ l("div", { id: a, children: g() })
  ] });
}, Zo = () => {
  const { audioToggledOn: A, toggleAudio: t } = Qt();
  return {
    audioToggledOn: A,
    toggleAudio: t
  };
}, Lo = () => {
  const { botIdRef: A } = q();
  return {
    getBotId: O(() => A.current, [A])
  };
}, Go = () => {
  const { getBotId: A } = Lo();
  return {
    getBotId: A
  };
}, Wo = () => {
  const { isChatWindowOpen: A, toggleChatWindow: t } = dA();
  return {
    isChatWindowOpen: A,
    toggleChatWindow: t
  };
}, Yo = () => {
  const { hasInteractedPage: A } = St();
  return {
    hasInteractedPage: A
  };
}, xo = () => {
  const { replaceMessages: A } = yA(), { replacePaths: t } = SA(), { replaceToasts: e } = CA(), { hasFlowStarted: o } = AA(), { flowRef: n } = q(), s = O(() => {
    A([]), e([]), t(["start"]);
  }, [A, e, t]), a = O(() => n.current ?? {}, [n]);
  return {
    hasFlowStarted: o,
    restartFlow: s,
    getFlow: a
  };
}, Xo = () => {
  const { hasFlowStarted: A, restartFlow: t, getFlow: e } = xo();
  return {
    hasFlowStarted: A,
    restartFlow: t,
    getFlow: e
  };
}, Jo = () => {
  const {
    endStreamMessage: A,
    injectMessage: t,
    removeMessage: e,
    streamMessage: o,
    messages: n,
    replaceMessages: s
  } = yA();
  return {
    endStreamMessage: A,
    injectMessage: t,
    removeMessage: e,
    streamMessage: o,
    messages: n,
    replaceMessages: s
  };
}, Vo = () => {
  const { settings: A, replaceSettings: t, updateSettings: e } = Mt();
  return {
    settings: A,
    replaceSettings: t,
    updateSettings: e
  };
}, Ko = () => {
  const { styles: A, replaceStyles: t, updateStyles: e } = Ht();
  return {
    styles: A,
    replaceStyles: t,
    updateStyles: e
  };
}, qo = () => {
  q();
  const {
    textAreaDisabled: A,
    toggleTextAreaDisabled: t,
    textAreaSensitiveMode: e,
    toggleTextAreaSensitiveMode: o,
    getTextAreaValue: n,
    setTextAreaValue: s,
    focusTextArea: a
  } = EA();
  return {
    textAreaDisabled: A,
    toggleTextAreaDisabled: t,
    textAreaSensitiveMode: e,
    toggleTextAreaSensitiveMode: o,
    getTextAreaValue: n,
    setTextAreaValue: s,
    focusTextArea: a
  };
}, _o = () => {
  const { showToast: A, dismissToast: t, toasts: e, replaceToasts: o } = CA();
  return {
    showToast: A,
    dismissToast: t,
    toasts: e,
    replaceToasts: o
  };
}, $o = () => {
  const { voiceToggledOn: A, toggleVoice: t } = LA();
  return {
    voiceToggledOn: A,
    toggleVoice: t
  };
}, An = () => {
  const { showChatHistory: A } = KA();
  return {
    showChatHistory: A,
    getHistoryMessages: YA,
    setHistoryMessages: mt
  };
};
export {
  J as Button,
  Oo as ChatBotProvider,
  jo as default,
  Qo as getDefaultSettings,
  So as getDefaultStyles,
  Zo as useAudio,
  Go as useBotId,
  An as useChatHistory,
  Wo as useChatWindow,
  Yo as useFirstInteraction,
  Xo as useFlow,
  Jo as useMessages,
  ro as useNotifications,
  ge as usePaths,
  Vo as useSettings,
  Ko as useStyles,
  qo as useTextArea,
  _o as useToasts,
  $o as useVoice
};
